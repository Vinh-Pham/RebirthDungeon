...
function M.learn(p, id)
    ...
    if p.skills[id] then return false end
...
function M.freeze(p)
    local ranks = {}
    for id, record in pairs(p.skills) do ranks[id] = record.rank end
    return ranks
end

function M.rank(p, id)
    id = Content.definitions[id] and Content.definitions[id].progression or id
    if p.run and p.run.skill_ranks then return p.run.skill_ranks[id] end
...
function M.migrate(p)
    if p.rules_version ~= 1 then return p end
    for _, record in pairs(p.skills) do
        record.legacy_training = record.training
        record.objectives = {}
    end
    if p.run then p.run.skill_ranks = M.freeze(p) end
    p.rules_version = 2
    return p
end

local function qualifies(id, objective, outcome)
    if objective.event == "normal_damage" then return outcome.skill == "normal" and outcome.damage > 0 end
    if objective.event == "skill_damage" then return outcome.skill == id and outcome.damage > 0 end
    if objective.event == "guard_use" then return outcome.skill == "defense" and outcome.hero_action end
    if objective.event == "guard_block" then return outcome.guarded and outcome.damage == 0 and not outcome.counter_negated end
    return false
end
    ...
    for id, learned in pairs(p.skills) do
        ...
        if rank and learned.last_training_action ~= outcome.id then
            for _, objective in ipairs(rank.objectives) do
                ...
                if count < objective.limit and qualifies(id, objective, outcome) and
                    (outcome.hero_action or objective.event == "guard_block") then
...
function M.can_advance(p, id)
    ...
    if not learned then return false, "Learn the skill first" end
    if learned.rank == #Content.ranks - 1 then return false, "Rank 1 is the maximum" end
    if p.phase ~= "town" or p.run then return false, "Advance in town, outside a run" end
    local next_rank = Content.definitions[id].ranks[learned.rank + 2]
    if not next_rank then return false, "Higher ranks are not available in this release" end
    if learned.training < 100 then return false, "Earn 100 training points first" end
    local cost = next_rank.ap[p.race]
    if p.ap < cost then return false, "Not enough AP", cost end
...
function M.validate_content(definitions)
    ...
    for id, d in pairs(defs) do
        ...
        U.require_ok(d.category == nil or U.contains({"melee", "ranged", "magic", "guns"}, d.category), "Invalid equipment category")
        U.require_ok(d.status == nil or d.status == "poison" or d.status == "armor_break", "Unsupported status effect")
        if d.giant_multiplier then U.require_ok(U.finite(d.giant_multiplier) and d.giant_multiplier > 0, "Invalid racial multiplier") end
        if d.kind == "attack" then
            U.require_ok(U.finite(d.multiplier) and d.multiplier > 0 and U.integer(d.hits, 1, 5), "Invalid attack effect")
            U.require_ok(d.base == nil or U.finite(d.base) and d.base >= 0, "Invalid base damage")
        end
        if d.progression then U.require_ok(defs[d.progression] and defs[d.progression].hero, "Missing progression skill") end
        if d.hero then
            ...
            for index, rank in ipairs(d.ranks) do
                ...
                for key, value in pairs(rank.giant_attributes or {}) do
                    U.require_ok(U.contains({"hp", "mp", "sp", "str", "int", "dex", "wil", "luk"}, key) and U.finite(value) and value >= 0, "Invalid racial attribute")
                end
                for key, value in pairs(rank.attributes) do
                    U.require_ok(U.contains({"hp", "mp", "sp", "str", "int", "dex", "wil", "luk"}, key) and U.finite(value) and value >= 0, "Invalid skill attribute")
                end
                for _, key in ipairs({"defense", "melee", "guard_defense", "guard_protection"}) do
                    if rank[key] then
                        for _, race in ipairs({"Human", "Elf", "Giant"}) do U.require_ok(U.finite(rank[key][race]) and rank[key][race] >= 0, "Invalid racial skill effect") end
                    end
                end
                local total, seen = 0, {}
                for _, objective in ipairs(rank.objectives) do
                    U.require_ok(type(objective.id) == "string" and not seen[objective.id], "Duplicate training objective")
                    U.require_ok(U.contains({"normal_damage", "skill_damage", "guard_use", "guard_block"}, objective.event), "Unsupported training objective")
                    U.require_ok(U.finite(objective.points) and objective.points > 0 and objective.points <= 100 and U.integer(objective.limit, 1, 10000), "Invalid training objective")
                    U.require_ok(objective.event ~= "skill_damage" or d.kind == "attack", "Unreachable damage objective")
                    U.require_ok((objective.event ~= "guard_use" and objective.event ~= "guard_block") or d.kind == "guard", "Unreachable guard objective")
                    seen[objective.id] = true; total = total + objective.points * objective.limit
                end
                U.require_ok(index == 15 or total >= 100, "Unreachable training rank")
                if index > 1 then
                    for _, race in ipairs({"Human", "Elf", "Giant"}) do U.require_ok(rank.ap and U.integer(rank.ap[race], 0, 1000000), "Missing next-rank AP cost") end
                end
            end
        end
    end
...
function M.validate_profile(p)
    for id, learned in pairs(p.skills) do
        ...
        U.require_ok(U.finite(learned.training) and learned.training >= 0 and learned.training <= 100, "Invalid skill training")
        if p.rules_version == 2 then
            U.require_ok(type(learned.objectives) == "table" and U.finite(learned.legacy_training) and learned.legacy_training >= 0 and learned.legacy_training <= 100, "Invalid training record")
            ...
            for key in pairs(learned.objectives) do U.require_ok(known[key], "Unknown training objective") end
            U.require_ok(learned.training == math.min(100, total), "Training total does not match objectives")
            U.require_ok(learned.last_training_action == nil or type(learned.last_training_action) == "string", "Invalid training action")
        end
    end
    if p.run and p.rules_version == 2 then
        U.require_ok(type(p.run.skill_ranks) == "table", "Missing frozen skill ranks")
        for id, rank in pairs(p.run.skill_ranks) do U.require_ok(p.skills[id] and p.skills[id].rank == rank, "Frozen rank differs from owned rank") end
        for id in pairs(p.skills) do U.require_ok(p.run.skill_ranks[id] ~= nil, "Missing frozen skill") end
    end
    return true
end
