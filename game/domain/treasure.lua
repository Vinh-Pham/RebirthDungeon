...
function M.migrate(p)
    ...
    if not run.treasure_version then
        ...
        if run.chests then
            for index,chest in ipairs(run.chests) do chest.opened=run.chosen==index end
        end
        run.chosen=nil
        if p.phase=="treasure" then
            p.phase="dungeon";local room=run.map.rooms[8];p.position={x=room.x*48,y=room.y*48}
        end
    end
    return p
end
