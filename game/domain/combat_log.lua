...
function M.trim(record, limit)
	while #record.events > limit do
		...
		until #record.events == 0 or record.events[1].operation ~= operation
...
local function metadata(p)
	...
	for _, room in ipairs(p.run.map.rooms) do
		if room.id == b.room then
			encounter = "Alby / " .. room.kind
		end
	end
	local participants = { { id = p.id, name = p.name, side = "hero" } }
	for _, actor in ipairs(b.enemies) do
		participants[#participants + 1] = { id = actor.id, name = actor.name, side = "enemy" }
	end
	return {
		id = b.id,
		run = p.run.id,
...
function M.sync(p)
	...
	if not record or record.id ~= b.id then
		record = metadata(p)
		p.combat_log.record = record
	end
	record.events = U.copy(b.events)
	record.sequence = b.sequence
	record.round = b.round
	record.turn = b.turn
	record.outcome = b.outcome or "active"
	record.incomplete = b.incomplete == true
end
...
function M.migrate(p)
	...
	U.require_ok(
		p.combat_log == nil or type(p.combat_log) == "table" and p.combat_log.version == M.VERSION,
		"Unsupported combat log format"
	)
	if p.format_version == 1 then
		p.format_version = 2
	end
	if p.combat_log then
		return p
	end
...
function M.emit(p, kind, text, fields)
	...
	p.last_events[#p.last_events + 1] = U.copy(event)
end
...
local function check(ok, reason)
	U.require_ok(ok, "Invalid save: combat log " .. reason)
end
local function events(list, battle, sequence, limit, strict)
	...
	for _, event in ipairs(list) do
		for key in pairs(event) do
			check(EVENT_FIELDS[key], "unknown event field")
		end
		check(
			bounded(event.battle, 128)
				and event.id == event.battle .. ":" .. event.sequence
				and U.integer(event.sequence, previous + 1, sequence or 10000000),
			"sequence"
		)
		...
		if strict then
			check(bounded(event.operation, 128) and bounded(event.run, 128), "operation/run identity")
		end
		check(
			(event.operation == nil or bounded(event.operation, 128)) and (event.run == nil or bounded(event.run, 128)),
			"operation/run fields"
		)
...
function M.validate(p)
	if p.combat_log == nil then
		check(p.format_version == 1, "missing schema")
		return
	end -- legacy save
	...
	if record then
		...
		check(
			type(record.participants) == "table" and #record.participants >= 1 and #record.participants <= 8,
			"participants"
		)
		local ids = {}
		for _, actor in ipairs(record.participants) do
			...
			check(
				bounded(actor.id, 128)
					and not ids[actor.id]
					and bounded(actor.name, 64)
					and (actor.side == "hero" or actor.side == "enemy"),
				"participant"
			)
			ids[actor.id] = true
		end
		...
		for _, event in ipairs(record.events) do
			check(
				event.run == record.run and event.turn <= record.turn and event.round <= record.round,
				"record boundary"
			)
			check(
				(not event.actor or ids[event.actor]) and (not event.target or ids[event.target]),
				"event participant"
			)
		end
		...
		if p.battle then
			check(
				record.id == p.battle.id
					and record.sequence == p.battle.sequence
					and U.equal(record.events, p.battle.events),
				"active history mismatch"
			)
		end
	else
		check(not p.battle, "missing active history")
	end
