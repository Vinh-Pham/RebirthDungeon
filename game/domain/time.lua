...
function M.civil(day)
	...
	local y = yo + era * 400
	local doy = doe - (365 * yo + math.floor(yo / 4) - math.floor(yo / 100))
...
local function sunday(y, m, n)
	local day = M.days(y, m, 1)
	local weekday = (day + 4) % 7
...
function M.latest(now)
	...
	day = day - (((day + 4) % 7 - 6) % 7)
	if M.noon(day) > now then
		day = day - 7
	end
	return day
end
function M.next_age(now)
	return M.noon(M.latest(now) + 7)
end
function M.cooldown(cumulative)
	if cumulative < 5000 then
		return 86400
	elseif cumulative < 8000 then
		return 2 * 86400
	elseif cumulative < 10000 then
		return 4 * 86400
	else
		return 6 * 86400
	end
