...
function M.occupied(p, bank)
    ...
    for _, item in ipairs(bank and p.bank.items or p.items) do
        if not item.slot then count = count + 1 end
    end
    return count
end
...
local function check_capacity(p, definition, quantity, bank)
    ...
    U.require_ok(M.occupied(p, bank) + math.ceil(remaining / d.stack) <= limit,
...
function M.add(p, definition, quantity, bank)
    ...
    while remaining > 0 do
        ...
        if d.durability then item.durability = d.durability end
        list[#list + 1] = item
        remaining = remaining - amount
    end
end
...
function M.remove(p, id, quantity, bank)
    ...
    if item.quantity == 0 then table.remove(bank and p.bank.items or p.items, index) end
end

function M.equipped(p, slot)
    for _, item in ipairs(p.items) do if item.slot == slot then return item end end
end
