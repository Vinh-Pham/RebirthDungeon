...
function M.name(name)
	...
	if #name < 2 or #name > 24 then
		return nil, "Use 2–24 characters"
	end
	if not name:match("^[A-Za-z][A-Za-z0-9 '%-]*$") then
		return nil, "Names use A–Z, numbers, spaces, apostrophes and hyphens"
	end
	return name, name:lower()
end
function M.new(id, name, race, age, talent, now, seed)
	local clean, reason = M.name(name)
	U.require_ok(clean, reason)
	U.require_ok(
		U.contains({ "Human", "Elf", "Giant" }, race) and U.integer(age, 10, 17) and C.talents[talent],
	...
	local p = {
		...
		created = now,
		reborn = now,
		age_boundary = T.latest(now),
		life = 1,
		next_id = 0,
		revision = 0,
		generation = 0,
		phase = "town",
		position = { x = 14 * 48, y = 9 * 48 },
		ledger = {},
		claims = {},
		quest_evidence = {},
		quest_claims = {},
		...
		dialogue = {},
		combat_log = { version = 1 },
		last_events = {},
		last_message = "Welcome to Town1. Speak with Aren at the northern gate.",
		random = { world = R.seed(seed, 54), reward = R.seed((seed + 17) % 4294967296, 91) },
	}
	for _, skill in ipairs({ "combat_mastery", "defense", C.talents[talent].skill }) do
		Skills.learn(p, skill)
	end
	I.add(p, C.talents[talent].weapon, 1)
...
function M.xp(p, amount)
	...
	while p.level < 200 and p.xp >= XP[p.level] do
		p.xp = p.xp - XP[p.level]
		p.level = p.level + 1
		p.cumulative = p.cumulative + 1
		p.ap = p.ap + 1
		for k, v in pairs(C.talents[p.talent].growth) do
			p.growth[k] = U.round((p.growth[k] or 0) + v, 4)
		end
	end
	if p.level == 200 then
		p.xp = 0
...
function M.age(p, now)
	...
	while p.age_boundary < latest do
		p.age_boundary = p.age_boundary + 7
		p.age = p.age + 1
		p.ap = p.ap + 5
		if p.age <= 20 then
			for k, v in pairs(C.talents[p.talent].aging) do
				p.growth[k] = U.round((p.growth[k] or 0) + v, 4)
			end
		end
	end
...
function M.rebirth(p, talent, age, now)
	...
	S.restore(p)
	p.last_message = "A new life begins. Your skills and possessions remain."
end
return M
