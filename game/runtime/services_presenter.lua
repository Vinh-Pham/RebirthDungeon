...
function M.model(p, request, locked)
    ...
    local model = {header = {service = request.service, name = service.name, title = service.title,
        tile = service.tile, gold = p.gold, carried = I.occupied(p), capacity = I.CARRIED_CAPACITY,
        quantity = quantity, dialogue = request.dialogue == true}, rows = {}, paragraphs = {}, choices = {}}
    if not Cmd.service_available(p, request.service) then locked = "Walk closer to the service in town" end
        ...
        if saved then
            ...
            for index, label in ipairs(saved.choices) do
                ...
                if intent and intent ~= "" then
                    ...
                    if command then enabled, reason = Cmd.preview_service(p, command) end
                    item.enabled = item.enabled and enabled; item.reason = locked or reason or ""
                end
                model.choices[#model.choices + 1] = item
            end
        end
        model.action = entry(p, "Start conversation again", {type = "dialogue", service = request.service, restart = true}, locked)
        return model
    end
    for _, id in ipairs(service.stock or {}) do
        ...
        local detail = item.power and ("Power +" .. item.power .. " · Durability " .. item.durability)
            or item.defense and ("Defense +" .. item.defense) or ("Restores " .. item.restore .. " " .. item.pool:upper() .. " when used")
        if item.no_giant then detail = detail .. " · Humans / Elves equip" end
        local action = entry(p, "Buy " .. quantity .. " · " .. item.price * quantity .. " gold",
            {type = "buy", service = request.service, def = id, quantity = quantity}, locked)
    ...
    if request.service == "healer" then
        ...
        model.header.effect = "Restore HP +" .. (maximum.hp - p.pools.hp) .. " · MP +" .. (maximum.mp - p.pools.mp) .. " · SP +" .. (maximum.sp - p.pools.sp)
        model.action = entry(p, "Full restoration · " .. service.cost .. " gold", {type = "heal"}, locked)
    elseif request.service == "gate" then
        model.header.effect = "Clear the encounters and boss, then spend its key in the treasure room. The goddess statue returns you home."
    ...
    elseif request.service == "blacksmith" then
        ...
        if weapon then
            ...
            model.action = entry(p, "Repair equipped weapon · " .. cost .. " gold", {type = "repair", item = weapon.id}, locked)
        end
    end
    return model
end
...
function M.send(p, request, receiver, locked)
    ...
    for _, row in ipairs(model.rows) do msg.post(receiver, "service_row", {token = request.token, row = row}) end
    for _, paragraph in ipairs(model.paragraphs) do msg.post(receiver, "service_paragraph", {token = request.token, text = paragraph}) end
    for _, choice in ipairs(model.choices) do msg.post(receiver, "service_choice", {token = request.token, entry = choice}) end
    if model.action then msg.post(receiver, "service_action", {token = request.token, entry = model.action}) end
