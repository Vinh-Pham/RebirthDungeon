...
function M.advance(saved, choice, service, restart)
    ...
    if restart then assert(not choice, "Restart before choosing a topic"); saved = nil end
    ...
    if saved then paragraphs, choices = story.restore(saved.state, false) else paragraphs, choices = story.continue() end
    local requested
    if choice then
    ...
    for _, paragraph in ipairs(paragraphs or {}) do text[#text + 1] = paragraph.text end
    ...
    if version == M.VERSION then result.service = service; result.actions = actions end
