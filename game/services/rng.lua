...
function M.valid(d)
    return type(d) == "table" and d.format_version == 1 and d.algorithm == "pcg32"
        and U.integer(d.seed_state, 0, 4294967295) and U.integer(d.seed_sequence, 0, 4294967295)
...
function M.open(descriptor, factory)
    ...
    factory = factory or function(a,b) assert(rng, "Native Defold RNG is required"); return rng.pcg32(a,b) end
    local native = factory(descriptor.seed_state, descriptor.seed_sequence)
    for _ = 1, descriptor.raw_draw_count do native:number() end
    local stream = {}
    function stream:raw()
        assert(descriptor.raw_draw_count < M.MAX_DRAWS, "Random stream replay limit exceeded")
        local n = native:number(); descriptor.raw_draw_count = descriptor.raw_draw_count + 1
        return n
    end
    function stream:int(lo, hi)
        ...
        local x = self:raw()
        while x >= limit do x = self:raw() end
        return lo + x % span
    end
    ...
    function stream:shuffle(a)
        for i = #a, 2, -1 do local j = self:int(1,i); a[i],a[j] = a[j],a[i] end
        return a
    end
    return stream
end
