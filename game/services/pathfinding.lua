...
function M.create(map,run)
    ...
    local cells={}
    for y=1,map.height do for x=1,map.width do cells[#cells+1]=D.walkable(map,x,y,run) and 1 or 0 end end
    astar.set_map(cells,id);astar.set_costs({[1]={1,1,1,1}},id)
    return id
end
function M.solve(id,sx,sy,ex,ey)
    local status,_,_,path=astar.solve(sx,sy,ex,ey,id)
    if status==astar.START_END_SAME then return {} end
    if status~=astar.SOLVED then return nil end
    return path
end
...
function M.dispose(id) if id then astar.delete_map(id) end end
return M
