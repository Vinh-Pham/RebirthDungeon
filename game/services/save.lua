...
local M = {}
local function newer(value, version)
	return type(value) == "number" and value > (version or 1)
end
...
function M.new(backend)
	...
	function self:load(id)
		...
		for _, suffix in ipairs({ "a", "b" }) do
			...
			if record then
				...
				if
					table_record
					and record.magic == "RebirthDungeon"
					and (
						newer(record.version)
						or payload
							and (newer(payload.format_version, 3) or newer(payload.rules_version, 2) or newer(
								payload.content_version
							) or type(payload.combat_log) == "table" and newer(payload.combat_log.version) or type(
								payload.run
							) == "table" and newer(payload.run.treasure_version))
					)
				then
					return nil, "A newer game version wrote this character", true
				end
				local ok = table_record
					and record.magic == "RebirthDungeon"
					and record.version == 1
					and U.integer(record.generation, 1, 1000000000)
					and payload
					and payload.id == id
					and payload.generation == record.generation
					and V.safe(payload)
				if ok then
					candidates[#candidates + 1] = { suffix = suffix, record = record }
				else
					errors[#errors + 1] = "Invalid slot " .. suffix
				end
			elseif exists or err then
				errors[#errors + 1] = err or "Unreadable slot " .. suffix
			end
		end
		...
		if #candidates > 0 then
			return Treasure.migrate(Log.migrate(Skills.migrate(U.copy(candidates[1].record.payload)))),
				#errors > 0 and "Recovered the previous valid save generation" or nil,
				true,
				candidates[1].suffix
		end
	...
	function self:commit(candidate)
		U.require_ok(
			candidate.rules_version == 2 and candidate.format_version == 3,
			"Migrate the character before saving"
		)
		V.profile(candidate)
		local previous, err, exists, slot = self:load(candidate.id)
		if exists and not previous then
			return nil, err
		end
		if previous and previous.revision >= candidate.revision then
			if previous.revision == candidate.revision then
				local same = U.copy(candidate)
				same.generation = previous.generation
				if U.equal(previous, same) then
		...
		local record =
			{ magic = "RebirthDungeon", version = 1, generation = candidate.generation, payload = U.copy(candidate) }
		local ok, result = pcall(backend.write, name, record)
		local observed = backend.read(name)
		if observed and U.equal(observed, record) and V.safe(observed.payload) then
			return true
		end
		return nil,
			ok and (result and "Saved data failed read-back validation" or "Storage write failed") or tostring(result)
	end
	function self:list()
		local profiles, errors = {}, {}
		-- Fixed stable slots make the index reconstructible without a separate pointer write.
		for i = 1, 20 do
			local id = string.format("profile_%02d", i)
			local p, err, exists = self:load(id)
			if p then
				profiles[#profiles + 1] = p
			end
			if err then
				errors[#errors + 1] = { id = id, error = err, recovered = not not p }
			end
		end
...
function M.defsave_backend(appname)
	...
	defsave.appname = appname or sys.get_config_string("storage.appname", "RebirthDungeon")
	defsave.use_default_data = false
	defsave.block_reloading = false
	defsave.verbose = false
	local backend = {}
	function backend.read(name)
		local path = defsave.get_file_path(name)
		...
		if not ok or type(data) ~= "table" or next(data) == nil then
			return nil, "Corrupt or unreadable save", true
		end
	...
	function backend.write(name, record)
		...
		for key, value in pairs(record) do
			defsave.set(name, key, U.copy(value))
		end
