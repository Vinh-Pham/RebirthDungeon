...
for _, id in ipairs(C.quest_order) do
	...
	config[id] =
		{ autostart = true, autofinish = false, tasks = { { action = d.action, object = d.object, required = 1 } } }
end
...
local function bind(state)
	...
	for _, q in pairs(state.current) do
		q.start_time = 0
	end
	quest.on_quest_event:clear()
end
	...
	local ok, result = pcall(function()
		...
		for _, e in ipairs(evidence or {}) do
			for _, id in ipairs(C.quest_order) do
				...
				if
					e.action == d.action
					and e.object == d.object
					and not p.quest_evidence[id]
					and (not d.previous or p.quest_evidence[d.previous])
				then
					quest.event(e.action, e.object, 1)
					p.quest_evidence[id] = e.id
				end
			end
		end
