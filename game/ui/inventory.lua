...
local function request(self)
    ...
    msg.post("/bootstrap#session", "inventory_request", {token = self.inventory_token,
        service = self.inventory_service or "", stored = self.inventory_stored or false,
        page = self.inventory_page or 1, item = self.inventory_item or "",
        quantity = tonumber(self.inventory_quantity) or 1, amount = tonumber(self.inventory_amount) or 1})
    self.dirty = true
end

local function submit(self, entry)
    if self.inventory_pending or not entry.enabled then return end
...
function M.on_message(self, id, message)
    ...
    if message.token ~= self.inventory_token then return false end
...
function M.cancel(self)
    if self.inventory_confirmation then
        self.inventory_confirmation = nil; self.dirty = true; return true
    end
    if self.inventory_item then
        self.inventory_item = nil; request(self); return true
    end
    return false
end
...
local function quantity_input(self, view, x, y, field, value, maximum)
    ...
    input.on_input_text:subscribe(function(_, entered) self[field] = entered end)
    view.button(self, x + 92, y, 76, "Set", function()
        ...
        if number and number % 1 == 0 and number >= 1 and number <= maximum then
            self.inventory_input_error = nil; request(self)
        else
...
local function action_button(self, view, x, y, width, entry)
    view.button(self, x, y, width, entry.label, function()
        if entry.command.type == "discard_item" then
            self.inventory_confirmation = entry; self.dirty = true
        else submit(self, entry) end
...
function M.render(self, view, context, service)
    ...
    if self.inventory_key ~= key then
        if self.inventory_service ~= service then
            self.inventory_page = 1; self.inventory_item = nil; self.inventory_stored = false
        end
        self.inventory_key = key; self.inventory_service = service
        request(self)
    end
    local model = self.inventory_data
    if not model then view.text(self, 115, 470, "Loading inventory…", 19, view.colors.muted); return end
    local h, detail = model.header, model.detail
    local c = view.colors
    if self.inventory_confirmation then
        view.text(self, 130, 465, "Discard " .. detail.quantity .. " × " .. detail.name .. "?", 25, c.white)
        view.text(self, 130, 407, "These items are removed permanently. There is no ground pickup or refund.", 18, c.muted)
        view.button(self, 345, 294, 350, "Confirm discard", function() submit(self, self.inventory_confirmation) end, not self.inventory_pending, true)
        view.button(self, 760, 294, 350, "Keep items", function() self.inventory_confirmation = nil; self.dirty = true end)
        return
    end
    ...
    if h.bank then
        view.button(self, 724, 536, 150, "Carried", function()
            self.inventory_stored = false; self.inventory_page = 1; self.inventory_item = nil; request(self)
        end, true, not h.stored)
        view.button(self, 919, 536, 220, "Bank " .. h.stored_count .. "/" .. h.bank_capacity, function()
            self.inventory_stored = true; self.inventory_page = 1; self.inventory_item = nil; request(self)
        end, true, h.stored)
    else view.text(self, 650, 536, h.gold .. " gold · Select an item for details", 16, c.muted) end
    for index, row in ipairs(model.rows) do
        ...
        local summary = row.summary .. (row.durability and " · Durability " .. row.durability .. "/" .. row.maximum or "")
        view.text(self, 168, y - 12, summary, 13, c.muted)
        local fn = function() select_item(self, row.id) end
        self.druid:new_button(node, fn)
        table.insert(self.buttons, {node = node, fn = fn, enabled = true, label = row.name})
    end
    if #model.rows == 0 then view.text(self, 124, 467, "No items here yet.", 19, c.muted) end
    view.button(self, 170, 166, 124, "Previous", function() self.inventory_page = h.page - 1; request(self) end, h.page > 1)
    view.text(self, 347, 166, h.page .. " / " .. h.pages, 16, c.muted, nil, false, true)
    view.button(self, 520, 166, 124, "Next", function() self.inventory_page = h.page + 1; request(self) end, h.page < h.pages)
    if detail then
        view.text(self, 636, 489, detail.name, 23, c.white, nil, true)
        view.text(self, 636, 459, detail.location .. (detail.durability and " · Durability " .. detail.durability .. "/" .. detail.maximum_durability or ""), 14, c.teal)
        ...
        view.button(self, 953, 306, 75, "−", function() self.inventory_quantity = tostring(math.max(1, detail.quantity - 1)); request(self) end, detail.quantity > 1)
        view.button(self, 1036, 306, 75, "+", function() self.inventory_quantity = tostring(math.min(detail.maximum_quantity, detail.quantity + 1)); request(self) end, detail.quantity < detail.maximum_quantity)
        view.button(self, 1119, 306, 75, "All", function() self.inventory_quantity = tostring(detail.maximum_quantity); request(self) end)
    ...
    else view.text(self, 636, 467, "Select an item to inspect it.", 19, c.muted) end
    if h.bank then
        ...
        for index, entry in ipairs(model.gold_actions) do action_button(self, view, 903 + (index - 1) * 174, 166, 158, entry) end
        view.text(self, 636, 128, disabled_reasons(model.gold_actions), 12, c.sp, 520)
    else view.text(self, 636, 172, "Tab / Enter: select and act · Escape: clear selection / close", 13, c.muted) end
    if self.inventory_input_error then view.text(self, 636, 131, self.inventory_input_error, 13, c.sp) end
end
