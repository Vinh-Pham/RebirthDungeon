...
function M.scroll(self, direction)
	...
	if not log or not log.available then
		return
	end
	local earlier, later = log.earlier, log.later
	if self.ui.modal ~= "log" then
		earlier = log.compact_earlier
		later = log.compact_later
	end
	if direction < 0 and earlier then
		M.move(self, earlier)
	elseif direction > 0 then
		if later then
			M.move(self, later)
		elseif not log.following then
			M.move(self, 0)
		end
	end
end
...
function M.render(self, view, log)
	...
	view.text(
		...
		118,
		534,
		log.encounter .. " · " .. log.outcome .. " · " .. log.origin .. " · Round " .. log.round,
		18,
		c.teal
	)
	local note = log.incomplete and "Partial history · Older complete operations were omitted."
		or "Latest encounter · Each row is one saved operation."
	if log.trimmed then
		note = "The position you were reading has left the retained history."
	end
	...
	if log.selected then
		for index, line in ipairs(log.details) do
			view.text(self, 756, 443 - (index - 1) * 23, line, 13, c.white)
		end
		view.button(self, 832, 226, 178, "Detail back", function()
			self.log_detail_page = log.detail_page - 1
			Bridge.request(self)
	...
	view.button(self, 221, 181, 210, "Earlier actions", function()
		M.move(self, log.earlier)
	end, log.earlier ~= nil)
	view.button(self, 455, 181, 210, "Later actions", function()
		M.move(self, log.later)
	end, log.later ~= nil)
	view.button(self, 703, 181, 250, "Jump to latest", function()
		M.move(self, 0)
	end, not log.following)
	view.text(
		self,
		854,
		181,
		log.new_entries > 0 and log.new_entries .. " new entries"
			or log.following and "Following latest"
			or "Reading earlier actions",
		13,
		c.muted
	)
end
