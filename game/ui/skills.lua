...
local function request(self)
	...
	msg.post("/bootstrap#session", "skills_request", {
		token = self.skills_token,
		group = self.skills_group or "All",
		page = self.skills_page or 1,
		skill = self.skills_selected or "",
...
function M.on_message(self, id, message)
	...
	if message.token ~= self.skills_token then
		return false
	end
	...
	elseif id == hash("skills_end") then
		self.skills_data = self.skills_incoming
		self.skills_incoming = nil
		self.dirty = true
	else
		return false
	end
...
function M.cancel(self)
	...
	if not self.skills_selected then
		return false
	end
	self.skills_selected = nil
	self.focus = self.skills_parent_focus
	request(self)
	return true
end

local function submit(self, action)
	if not action.enabled or self.skills_pending then
		return
	end
...
function M.render(self, view, context)
	...
	if not model then
		view.text(self, 112, 477, "Loading skills…", 18, view.colors.muted)
		return
	end
	...
	for index, row in ipairs(model.rows) do
		...
		view.text(
			...
			row.training
				.. " / 100"
				.. (row.alias and " · Combat Mastery" or row.kind == "passive" and " · Passive" or " · Active"),
			14,
			c.muted
		)
		local fn = function()
			select_skill(self, row.id)
		end
	...
	if #model.rows == 0 then
		view.text(
			...
			112,
			460,
			h.group == "Life" and "Life skills are reference-only for now." or "No learned skills in this category.",
			17,
			c.muted,
			400
		)
	end
	view.button(self, 171, 168, 130, "Previous", function()
		self.skills_page = h.page - 1
		request(self)
	end, h.page > 1)
	view.text(self, 309, 168, h.page .. " / " .. h.pages, 15, c.muted, nil, false, true)
	view.button(self, 449, 168, 130, "Next", function()
		self.skills_page = h.page + 1
		request(self)
	end, h.page < h.pages)
	...
	if not detail then
		...
		view.text(
			...
			556,
			411,
			"Inspect effects, exact costs, requirements and training. Attack shares Combat Mastery's progression.",
			18,
			c.muted,
			590
		)
	...
	view.text(
		...
		555,
		481,
		detail.name .. " · Rank " .. detail.rank .. " · " .. detail.training .. " / 100",
		23,
		c.white,
		nil,
		true
	...
	for index, objective in ipairs(model.objectives) do
		view.text(
			...
			555,
			297 - (index - 1) * 23,
			objective.count .. "/" .. objective.limit .. " · " .. objective.points .. " points · " .. objective.text,
			12,
			c.muted
		)
	end
	...
	view.button(self, 1007, 210, 276, "Use skill", function()
		submit(self, detail.use)
	end, detail.use.enabled and not self.skills_pending, true)
	view.text(self, 555, 175, detail.advance.reason, 11, c.sp, 285)
	view.text(
		self,
		869,
		175,
		detail.use.enabled and ("Target: " .. detail.use.target) or detail.use.reason,
		11,
		c.sp,
		285
	)
	view.button(self, 714, 579, 160, "Back to list", function()
		M.cancel(self)
	end)
	view.button(self, 1057, 579, 180, "Source / learning", function()
		self.skills_show_source = not self.skills_show_source
		self.dirty = true
	end)
	if self.skills_show_source then
		...
		view.text(
			...
			560,
			266,
			"Only authored starter values are executable. Higher source ranks are not installed.",
			12,
			c.muted
		)
	end
