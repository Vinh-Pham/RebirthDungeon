...
3. Run `claude` in the project folder (or open Cursor) → it connects. Try *"list_skills"*.
...
## 2. Enable the plugin
Open the project in Godot → **Project → Project Settings → Plugins** → enable **Beckett — MCP for Godot**.
...
## 5. Try it
Ask the agent: *"call get_godot_version"*, *"get_scene_tree"*, or *"list_skills"*. For a full loop: *"create a Button, play the scene, screenshot it, then click it."*
...
- **Everything else is stripped at export time.** Beckett registers an `EditorExportPlugin`
  that drops every other addon file from the pack. A typical export goes from ~480 KB of
  Beckett down to a 1 KB stub, and the export log says so:

  ```
  [beckett] kept 55 editor-only file(s) (750.7 KiB) out of the export; only the inert runtime autoload ships
  ```
