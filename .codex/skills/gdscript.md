...
**Edit `.gd` files ONLY via `write_script` / `script_patch`** (they make the editor reload the script). A file changed any other way (your own file tools, an external editor) stays STALE in a driven editor: reload-on-focus never fires, so `call_method` runs the OLD code and reports success. Beckett attaches a `warning` to the response when it detects the drift; rewrite via `write_script` to clear it.

On Godot 4.5+ any tool response may also carry `engine_errors`: the engine raised errors while that call ran. The call completed, but treat its effect as unverified until you have read them.
