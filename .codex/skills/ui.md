...
## Verify at runtime
`play_scene` → **`ui_snapshot`** first (1.10+): ONE call returns every visible Control with rect + semantic state (disabled / focused / checked / value / selected / editable / text), honesty flags (`clipped`, `occluded_by`), the focus owner, open popups, and a content `hash` — pass it back as `since_hash` and an unchanged UI answers `{unchanged:true}` for ~free. Read screens from it instead of screenshots; when you DO need eyes, `screenshot annotate=ui scale=0.5 format=jpeg` gives a numbered Set-of-Mark image + a `{marks}` legend mapping numbers to node paths at a fraction of the PNG cost. Then drive: `click_control text="Play"` (auto-waits for clickability; refuses disabled/occluded clicks with the reason — never a fake success), `type_text text="hero" name=NameEdit submit=true` (real per-char key events, so `text_changed`/`text_submitted` fire), `click_button_by_text` for the fast signal-only path, `simulate_input` for keyboard/focus navigation; then `wait_for_node` / `monitor_properties` / `assert_node_state` to check state. A whole flow fits ONE call: `ui_do steps=[{"click":"Options"},{"wait":{"node":"OptionsMenu"}},{"type":{"name":"NameEdit","text":"hero"}},{"assert":{"condition":"..."}}]` — each step auto-waits, asserts poll until true (eventually semantics), the first failure names its blocker. Finish with **`ui_audit touch_min=44`**: measured layout QA (offscreen / overlapping / font-measured text overflow / sub-44px touch targets / mouse-only buttons a gamepad can't reach) plus the focus GRAPH (1.11+): `no_initial_focus` (nothing has focus - a gamepad user can't start navigating), `focus_unreachable` (no Tab/arrow chain from the entry reaches it), `focus_dead_end` (focus enters but can't leave), with a `focus: {focusable, entry, reachable}` summary. For typing, `type_text per_frame=true` (1.11+) streams one char per frame when handlers count keystrokes. On older Beckett (<1.10) fall back to `find_ui_elements` / `get_remote_tree` + `get_control_rect`.
