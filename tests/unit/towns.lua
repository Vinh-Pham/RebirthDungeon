...
local function command(p, c, env)
	sequence = sequence + 1
	c.op = c.op or "town_test_" .. sequence
	c.revision = c.revision or p.revision
	return Cmd.execute(p, c, env or ENV)
end

local function story(_, choice, service)
	return { version = 2, service = service, state = {}, paragraphs = { "Elara" }, choices = {}, actions = {} },
		choice and "heal" or nil
end
...
describe("Town services contract", function()
	it("shares authored occupancy between art and navigation and crosses the stream only at the bridge", function()
		...
		expect.equal(Town.cell(12, 9), "bridge")
		expect.equal(Town.cell(12, 11), "stream")
		expect.equal(Town.cell(14, 9), "square")
		expect.equal(Town.cell(14, 16), "road")
		for _, id in ipairs(C.service_order) do
			local npc = C.services[id]
			expect.truthy(D.walkable(map, npc.x, npc.y))
		end
	end)

	it("requires current town proximity for every service and rejects old character commands", function()
		local p = fresh("gate")
		expect.truthy(Cmd.service_available(p, "gate"))
		expect.falsy(Cmd.service_available(p, "healer"))
		...
		for _, id in ipairs(C.service_order) do
			expect.falsy(Cmd.service_available(p, id))
		end
	end)
	...
	it("previews exact restoration without healing or spending during panel display", function()
		...
		expect.truthy(model.action.enabled)
		expect.equal(model.action.label, "Full restoration · 10 gold")
		...
		expect.falsy(model.action.enabled)
		expect.equal(model.action.reason, "Already fully restored")
	end)
	...
	it("disables unaffordable, full-pack and remote purchases with authoritative reasons", function()
		...
		expect.truthy(model.rows[1].action.enabled)
		expect.falsy(model.rows[4].action.enabled)
		I.add(p, "armor", 30)
	...
	it("rejects invalid purchase quantities atomically and keeps previews RNG-free", function()
		...
		for _, id in ipairs(C.service_order) do
			Presenter.model(p, { service = id })
		end
		expect.truthy(U.equal(before, p))
		p = command(p, { type = "buy", service = "shop", def = "hp_potion", quantity = 3 })
		expect.equal(p.gold, 70)
		expect.equal(p.items[2].quantity, 3)
	...
	it("does not advance a spending dialogue choice when funds or save writes fail", function()
		...
		expect.equal(store:load(p.id).gold, 90)
	...
	it("uses the same disabled reasons for dialogue purchases and normal service buttons", function()
		...
		expect.falsy(model.choices[1].enabled)
		expect.equal(model.choices[1].reason, "Not enough gold")
		p.gold = 5
		model = Presenter.model(p, { service = "grocery", dialogue = true })
	...
	it("returns a defeated hero to Elara for free with claimed possessions and experience intact", function()
		...
		while battle.active(p).hero do
			p.battle.cursor = p.battle.cursor + 1
		end
		...
		expect.truthy(Cmd.service_available(p, "healer"))
		expect.equal(p.gold, 147)
		expect.equal(p.bank.gold, 12)
		expect.equal(p.xp, 77)
		expect.equal(p.items[2].quantity, 4)
		expect.falsy(p.quest_evidence.return_home)
		for _, pool in ipairs({ "hp", "mp", "sp" }) do
			expect.equal(p.pools[pool], Stats.current(p)[pool])
		end
	end)
	...
	it("previews the gate without generating a dungeon and requires explicit abandonment", function()
		...
		p.gold = 143
		I.add(p, "silk", 4)
		local experience = p.xp
		p = command(p, { type = "abandon", confirm = true })
		expect.equal(p.gold, 143)
		expect.equal(p.xp, experience)
		expect.falsy(p.run)
		expect.falsy(p.quest_evidence.return_home)
		expect.truthy(Cmd.service_available(p, "healer"))
		expect.equal(p.pools.hp, Stats.current(p).hp)
	...
	it("leaves only a confirmed current run at its entrance and saves the outside position atomically", function()
		...
		local distant = U.copy(p)
		distant.position.x = distant.position.x + 480
		...
		expect.falsy(result.quest_evidence.return_home)
		backend.mode = "fail"
		...
		expect.truthy(U.equal(loaded, result))
		local retry, duplicate = Cmd.execute(loaded, leave, ENV)
		expect.truthy(duplicate)
		expect.truthy(U.equal(retry, loaded))
	end)
end)
