...
local function command(p, c)
	sequence = sequence + 1
	c.op = c.op or "skills_test_" .. sequence
...
local function legacy(p)
	p.rules_version = 1
	for _, learned in pairs(p.skills) do
		learned.objectives = nil
		learned.legacy_training = nil
		learned.last_training_action = nil
	end
	if p.run then
		p.run.skill_ranks = nil
	end
	return p
end
...
describe("Starter skills contract", function()
	...
	it("makes duplicate learning a no-op and preserves rebirth progress without equipment gifts", function()
		...
		expect.falsy(K.learn(p, "smash"))
		expect.truthy(U.equal(p, before))
		local items, ap = U.copy(p.items), p.ap
		Ch.rebirth(p, "Magic", 10, ENV.now + 86400)
		expect.equal(p.skills.smash.training, 5)
		expect.equal(p.skills.icebolt.training, 0)
		expect.equal(p.ap, ap)
		expect.truthy(U.equal(items, p.items))
		expect.falsy(pcall(K.learn, p, "normal"))
		expect.falsy(pcall(K.learn, p, "pounce"))
		expect.falsy(pcall(K.learn, p, "wand_mastery"))
		expect.falsy(pcall(K.learn, fresh("Giant"), "power_shot"))
	end)
	it("validates objective reachability and rejects fabricated costs and effects", function()
		...
		defs = U.copy(Content.definitions)
		defs.icebolt.cost = 0 / 0
		expect.falsy(pcall(K.validate_content, defs))
		defs = U.copy(Content.definitions)
		defs.defense.kind = "counter"
		expect.falsy(pcall(K.validate_content, defs))
		defs = U.copy(Content.definitions)
		defs.smash.ranks[1].attributes.str = math.huge
		expect.falsy(pcall(K.validate_content, defs))
		defs = U.copy(Content.definitions)
		defs.normal.progression = "missing"
		expect.falsy(pcall(K.validate_content, defs))
	end)
	...
	it("credits Double Shot once per action, never once per hit or repeated operation", function()
		...
		expect.truthy(U.equal(retry, after))
	...
	it("does not train from menus, previews, failed actions or blocked damage", function()
		...
		expect.truthy(U.equal(before, p))
		local foreign = action(p, "smash")
		foreign.profile = "profile_02"
		expect.falsy(pcall(command, p, foreign))
		expect.truthy(U.equal(before, p))
		local c = action(p, "smash")
		c.target = p.id
		expect.falsy(pcall(command, p, c))
		expect.truthy(U.equal(before, p))
		p.battle.statuses[p.battle.enemies[1].id] = { guard = { defense = 1000, protection = 100 } }
		p = command(p, action(p, "normal"))
		expect.equal(p.skills.combat_mastery.training, 0)
	end)
	...
	it("bounds each objective and total, with one credit per resolved action identity", function()
		...
		expect.truthy(V.safe(p))
		p.skills.combat_mastery.objectives.damaging_attack = 21
	...
	it("does not infer playable higher ranks, enemy skills or a separate Attack record", function()
		...
		expect.truthy(U.equal(before, p))
		p.skills.smash.rank = 1
		expect.falsy(V.safe(p))
		p = fresh()
		p.skills.normal = U.copy(p.skills.smash)
		expect.falsy(V.safe(p))
		p = fresh()
		p.skills.pounce = U.copy(p.skills.smash)
		expect.falsy(V.safe(p))
	end)
	it("applies cumulative F-rank bonuses once and mastery attack bonuses only to melee", function()
		for _, race in ipairs({ "Human", "Elf", "Giant" }) do
			...
			K.learn(p, "icebolt")
			expect.truthy(U.equal(after, S.profile(p)))
			local mastery = race == "Human" and 1.5 or 1
			expect.equal(after.melee, math.floor(after.str * 0.1) + mastery)
			expect.equal(after.ranged, math.floor(after.dex * 0.1))
			expect.equal(after.guns, math.floor((after.str + after.int) * 0.05))
		end
	end)
	it("migrates old aggregate training and frozen runs without rewriting the old slot", function()
		local p = legacy(encounter(fresh()))
		p.skills.smash.training = 35
		p.revision = 4
		p.generation = 1
		local before = U.copy(p)
		...
		local migrated = store:load(p.id)
		expect.equal(migrated.rules_version, 2)
		expect.equal(migrated.skills.smash.training, 35)
		expect.equal(migrated.skills.smash.legacy_training, 35)
		...
		expect.truthy(store:commit(migrated))
		expect.equal(store:load(p.id).skills.smash.training, 40)
	end)
	...
	it("shows categories, exact costs, requirements and authoritative disabled reasons", function()
		...
		expect.truthy(model.detail.alias)
		expect.equal(model.detail.rank, "F")
		expect.falsy(model.detail.use.enabled)
		expect.falsy(model.detail.advance.enabled)
		p = encounter(fresh("Human", "Magic"))
		model = Presenter.model(p, { group = "Magic", skill = "icebolt" })
		expect.equal(#model.rows, 1)
		expect.truthy(model.detail.use.enabled)
		expect.truthy(model.detail.cost:match("1 MP once"))
		p.pools.mp = 0
		model = Presenter.model(p, { group = "Magic", skill = "icebolt" })
		expect.falsy(model.detail.use.enabled)
		expect.equal(model.detail.use.reason, "Not enough MP")
		model = Presenter.model(p, { group = "All", skill = "normal" }, "Resolve the save failure first")
		expect.falsy(model.detail.use.enabled)
		expect.equal(model.detail.use.reason, "Resolve the save failure first")
	end)
end)
