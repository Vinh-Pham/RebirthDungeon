...
describe("Inventory contract", function()
	...
	it("presents all 60 bank slots in bounded pages with exact gold controls", function()
		...
		p.position = { x = 8 * 48, y = 6 * 48 }
		local model = Presenter.model(p, { service = "bank", stored = true, page = 2, amount = 37 })
		expect.equal(#model.rows, 30)
		expect.equal(model.header.pages, 2)
		expect.equal(model.rows[30].id, p.bank.items[60].id)
	...
	it("presents the nine equipment slots and keeps equipped items out of the item grid", function()
		local p = fresh()
		I.add(p, "armor", 1)
		I.equip(p, p.items[2].id)
		local model = Presenter.model(p, {})
		expect.equal(#model.equipment, 9)
		expect.equal(model.equipment[1].slot, "accessory_1")
		expect.equal(model.equipment[2].slot, "head")
		expect.equal(model.equipment[3].slot, "accessory_2")
		expect.equal(model.equipment[4].slot, "weapon")
		expect.equal(model.equipment[4].name, "Traveler's Sword")
		expect.equal(model.equipment[5].slot, "body")
		expect.equal(model.equipment[5].name, "Leather Coat")
		expect.equal(model.equipment[6].slot, "hand_left")
		expect.equal(model.equipment[9].slot, "robe")
		expect.equal(model.equipment[9].name, "")
		expect.equal(#model.rows, 0, "Every owned item is equipped")
		model = Presenter.model(p, { service = "bank" })
		expect.falsy(model.equipment, "The bank has no equipment slots")
	end)
end)
