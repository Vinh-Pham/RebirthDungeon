...
local function rewards(p)
    ...
    p.pending = {id = "reward_fixture", origin = "encounter", entries = {
...
describe("Inventory contract", function()
    ...
    it("does not spend gold on an overflowing multi-item purchase", function()
        local p = fresh()
        I.add(p, "hp_potion", 98); I.add(p, "armor", 29)
    ...
    it("moves exact character-specific gold and requires a direction", function()
        local p = fresh()
        local other = fresh(); other.id = "profile_02"
        p.position = {x = 8 * 48, y = 6 * 48}
        p = command(p, {type = "bank_gold", amount = 37, deposit = true})
        expect.equal(p.gold, 63); expect.equal(p.bank.gold, 37)
        p = command(p, {type = "bank_gold", amount = 12, deposit = false})
        expect.equal(p.gold, 75); expect.equal(p.bank.gold, 25)
        ...
        expect.equal(other.gold, 100); expect.equal(other.bank.gold, 0)
    end)

    it("repairs carried or equipped weapons at one gold per missing point", function()
        local p = fresh(); I.add(p, "wand", 1)
        p.items[1].durability = 0; p.items[2].durability = 13
        p.position = {x = 19 * 48, y = 6 * 48}
        p = command(p, {type = "repair", item = p.items[1].id})
        p = command(p, {type = "repair", item = p.items[2].id})
        expect.equal(p.gold, 73)
        expect.equal(p.items[1].durability, 20); expect.equal(p.items[2].durability, 20)
    ...
    it("spends one durability per committed attack and keeps unarmed attacks legal", function()
        ...
        local c = {type = "act", actor = p.id, battle = p.battle.id, turn = p.battle.turn,
            skill = "normal", target = p.battle.enemies[1].id}
        p = command(p, c)
        ...
        p = fresh(); I.equip(p, p.items[1].id); p = battle(p)
        p = command(p, {type = "begin_turn", battle = p.battle.id, turn = p.battle.turn})
    ...
    it("claims selected entries in offer order and leaves overflow offers available", function()
        local p = rewards(fresh()); I.add(p, "armor", 29)
        rejected(p, {type = "claim", reward = p.pending.id, ids = {"gold_fixture", "silk_fixture", "potion_fixture"}})
        p = command(p, {type = "claim", reward = p.pending.id, ids = {"silk_fixture"}})
        expect.truthy(p.pending.entries[2].claimed)
        expect.falsy(p.pending.entries[1].claimed); expect.falsy(p.pending.entries[3].claimed)
        rejected(p, {type = "use_item", item = p.items[1].id})
        rejected(p, {type = "leave_rewards"})
        local a = rewards(fresh()); local b = U.copy(a)
    ...
    it("requires explicit discard quantity and confirmation, preserving items on failed writes", function()
        ...
        p.revision = 1; expect.truthy(store:commit(p))
        local before = U.copy(p)
        local candidate = command(p, {type = "discard_item", item = id, quantity = 3, confirm = true})
        backend.mode = "fail"
        expect.falsy(store:commit(candidate))
        expect.truthy(U.equal(store:load(p.id), before)); expect.truthy(U.equal(p, before))
    ...
    it("keeps reward claims, gold and items together across a failed write and retry", function()
        local p = rewards(fresh()); p.revision = 4
        local backend = Support.backend(); local store = Save.new(backend)
        ...
        expect.equal(reloaded.gold, 112); expect.truthy(reloaded.claims.silk_fixture)
        expect.equal(reloaded.phase, "dungeon"); expect.falsy(reloaded.pending)
        expect.falsy(reloaded.claims.potion_fixture)
        local retry, duplicate = Cmd.execute(reloaded, c, ENV)
        expect.truthy(duplicate); expect.truthy(U.equal(retry, reloaded))
    end)

    it("rejects duplicated ownership, sparse lists and invalid frozen equipment", function()
        local p = fresh(); p.bank.items[1] = U.copy(p.items[1]); p.bank.items[1].slot = nil
        expect.falsy(V.safe(p))
        p = fresh(); p.items[20] = p.items[1]; expect.falsy(V.safe(p))
        p = fresh(); I.add(p, "silk", 1); p.items[2].durability = 20; expect.falsy(V.safe(p))
        p = battle(fresh()); p.run.baseline.weapon_id = "missing"; expect.falsy(V.safe(p))
    end)

    it("previews exact commands without mutating character, quest binding or RNG", function()
        local p = fresh(); I.add(p, "hp_potion", 2)
        local before = U.copy(p); local binding = Q.bound_state()
    ...
    it("presents all 60 bank slots in bounded pages with exact gold controls", function()
        local p = fresh(); I.add(p, "armor", 60, true)
        p.position = {x = 8 * 48, y = 6 * 48}
        local model = Presenter.model(p, {service = "bank", stored = true, page = 10, amount = 37})
        expect.equal(#model.rows, 6); expect.equal(model.header.pages, 10)
