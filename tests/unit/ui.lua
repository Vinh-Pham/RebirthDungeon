...
local function state(p)
	return {
		screen = p and p.phase or "title",
		settings = { music = 0.4, effects = 0.65, reduced_motion = false, hud_scale = 1.3 },
		profiles = p and { p } or {},
		epoch = 1,
	}
end
...
describe("UI projections", function()
	it("provides neutral title data and exposes no save/RNG/treasure internals", function()
		...
		expect.falsy(model.profile)
		expect.equal(model.screen, "title")
		local p = battle()
		model = UI.model(p, state(p), {}, NOW)
		expect.falsy(model.profile.random)
		expect.falsy(model.profile.ledger)
		expect.falsy(model.profile.run.map)
		expect.falsy(model.profile.run.chests)
		expect.falsy(model.profile.battle.random)
		expect.falsy(model.profiles[1].items)
	end)
	it("shows exact fractional costs and turn/cooldown reasons without advancing turns", function()
		...
		p = Cmd.execute(
			p,
			{ type = "begin_turn", battle = p.battle.id, turn = p.battle.turn, op = "ui_begin", revision = p.revision },
			ENV
		)
	...
	it("previews selected reward capacity independently from Take All and preserves offers", function()
		...
		expect.falsy(model.profile.pending.all.enabled)
		expect.truthy(model.profile.pending.selected.enabled)
		expect.equal(#model.profile.pending.selected.command.ids, 1)
		expect.truthy(U.equal(before, p))
		expect.equal(Q.bound_state(), binding)
		local take = model.profile.pending.selected.command
		take.op = "take_selected"
		local result = Cmd.execute(p, take, ENV)
		expect.equal(result.phase, "dungeon")
		expect.falsy(result.pending)
	...
	it("projects current stats, frozen-run explanations and rebirth eligibility", function()
		...
		expect.truthy(model.profile.rebirth.seconds > 0)
	...
	it("bounds log pages and supplies map cells without mutating gameplay", function()
		local p = battle()
		for index = 1, 35 do
			p.battle.events[index] =
				{ sequence = index, operation = "op_" .. index, kind = "wait", text = "Example", turn = 1, round = 1 }
		end
		p.battle.sequence = 35
		require("game.domain.combat_log").sync(p)
		local shell = state(p)
		shell.modal = "log"
		local model = UI.model(p, shell, { log_anchor = 5 }, NOW)
		expect.equal(model.profile.combat_log.last, 5)
		expect.equal(#model.profile.combat_log.rows, 5)
		p = fresh()
		shell = state(p)
		shell.modal = "map"
		local before = U.copy(p)
		model = UI.model(p, shell, {}, NOW)
		expect.equal(#model.map.points, 6)
		expect.truthy(#model.map.cells > 0)
		expect.truthy(U.equal(before, p))
	end)
end)
