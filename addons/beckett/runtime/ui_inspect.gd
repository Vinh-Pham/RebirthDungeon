...
static func pick_at(vp: Viewport, tree_root: Node, p: Vector2) -> Node:
	...
	for i in range(wins.size() - 1, -1, -1):
		var w: Window = wins[i]
...
const MARK_COLOR := Color(1.0, 0.0, 1.0)
...
static func _outline(img: Image, r: Rect2i, w: int) -> void:
	img.fill_rect(Rect2i(r.position, Vector2i(r.size.x, w)), MARK_COLOR)
	img.fill_rect(Rect2i(Vector2i(r.position.x, r.end.y - w), Vector2i(r.size.x, w)), MARK_COLOR)
	img.fill_rect(Rect2i(r.position, Vector2i(w, r.size.y)), MARK_COLOR)
	img.fill_rect(Rect2i(Vector2i(r.end.x - w, r.position.y), Vector2i(w, r.size.y)), MARK_COLOR)
