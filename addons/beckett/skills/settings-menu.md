...
play_scene → click_button_by_text "Fullscreen" → screenshot
# Verify what you PERSISTED, not the window: runtime_call path=/root/Settings method=fetch args=["video", "mode", 0] → 3.
# window_get_mode() stays WINDOWED while the game runs embedded in the editor (see Common traps).
```
...
- **An embedded game view can never change window mode.** Since 4.4 the editor plays the game INSIDE its Game workspace by default, and both placements count as embedded ("Make Game Workspace Floating" is a floating workspace, not a real OS window). An embedded window ignores `window_set_mode`, so `DisplayServer.window_get_mode()` keeps answering WINDOWED however correct your menu code is: asserting on it sends you hunting a phantom bug. Assert the value you PERSISTED instead (`runtime_call path=/root/Settings method=fetch args=["video", "mode", 0]`, or read the ConfigFile), or check the real window by running unembedded (Editor Settings > Run > Window Placement > Game Embed Mode = Disabled) or from an export. `doctor` says which one you are in: `game_view.mode` (`embedded` / `windowed` / `unknown`) and `game_view.placement` (`main` or `floating` when embedded, `own_window` when not), plus a `source` line naming the setting it read.
...
- VSYNC_MAILBOX / EXCLUSIVE_FULLSCREEN can silently fall back — read back `window_get_vsync_mode`/`window_get_mode` if you reflect the true state; cap fps with `Engine.max_fps`. (The mode read-back says nothing while the game runs embedded, per the trap above; the vsync one is still honest.)
