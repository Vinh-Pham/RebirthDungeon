...
4. **Act:**
   ...
   - `call_method target=<node> method=<Name> args=[...]`: args coerce to the declared param types (vectors `[x,y,z]` / `{"x":..}` / `"x y z"`; colors hex or `[r,g,b,a]`; StringName/NodePath from strings; object params take a node name/path or `res://` path). Wrong type/count returns an ERROR, never a silent no-op: read the message, fix the arg.
   - `connect_signal from=<a> signal=<s> to=<b> method=<m>`.

## `target` can be
- a node name/path in the open scene: `Player`, `UI/HealthBar`
- the scene root itself: `.`, `/root`, or the root's own name
- a `res://` resource path
