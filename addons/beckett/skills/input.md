...
## Driving input from the agent (`simulate_input` / `type_text` / `record_input` / `replay_input`)
`simulate_input` injects these event types into the running game: `key` (optional `unicode` codepoint so a key event can CARRY TEXT, 1.10+), `action`, `mouse_button`, `mouse_motion`, `joy_button` (`{button, pressed, device}`, button = `JOY_BUTTON_*` index), `joy_axis` (`{axis, value, device}`, axis = `JOY_AXIS_*` index, value clamped -1..1), `touch` (`{index, position, pressed}`), `touch_drag` (`{index, position, relative}`). For text fields prefer `type_text` (1.10+): it focuses the target and types the string as real per-char unicode key events, so `text_changed`/`text_submitted`/`max_length` behave like a player typed. `record_input`/`replay_input` capture and replay the full set (gamepad, touch, and typed-text unicode included), so a manual run round-trips.
**Frame-exact input (1.12+):** `time_control op=step frames=N inputs_per_frame=[[ev,...], [], ...]` injects each array on its own physics tick inside the stepped window, so "hold jump for 3 frames, release on frame 4" is reproducible. Injecting between separate tool calls lands on whatever frame the round-trip happens to hit, which is not a test. **Measured timing:** `_input` sees an event on its own tick; polled state (`Input.is_key_pressed` / `is_action_pressed`) only reflects it from the NEXT tick - assert against whichever half your game actually reads.

`type_text per_frame=true` (1.11+) streams one char per frame - use it when handlers count keystrokes (`text_changed` per char, typing minigames, rate limits); default one-frame typing coalesces `text_changed` into one emission (paste semantics).
