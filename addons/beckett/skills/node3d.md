...
## Triangle winding — Godot's front face is CLOCKWISE
Opposite of the OpenGL convention most mesh-generation code is written against, and with the default `CULL_BACK` a reversed index buffer makes the mesh **silently vanish**: no error, no warning, clean log, `visible = true`, correct AABB, node present in the tree. The classic symptom is a surface you can only see from behind, or a "ground" that turns out to be sky below the horizon.
- Building an `ArrayMesh` by hand (`add_surface_from_arrays`) or a `SurfaceTool`: emit each triangle's indices so it reads clockwise from the side you want visible. The outward normal of `(v0, v1, v2)` is `(v2 - v0).cross(v1 - v0)` — that order, not the habitual one. `SurfaceTool.generate_normals()` derives normals from the winding, so wrong winding gives you consistently wrong normals too.
- `PrimitiveMesh.flip_faces` / `SurfaceTool.generate_normals(true)` flip the sense if you need the other side.
- Confirm instead of guessing: `render_probe path=<node>` reports `winding.facing_camera` (0 of N sampled = reversed) and `winding.vs_normals`; `set_debug_draw mode=wireframe` draws geometry regardless of culling, so anything visible there but nowhere else is a culling problem. Setting `cull_mode = 2` temporarily confirms it in one step — then fix the winding and put culling back, since shipping with it disabled doubles fragment cost.
