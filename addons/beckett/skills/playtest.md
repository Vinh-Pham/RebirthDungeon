...
2. `record_input action=start`, drive the game (`simulate_input`, `click_control`, `type_text`, or a human at the keyboard), `record_input action=stop` - you get an `events` array, each event stamped with `t` (ms) and `f` (physics frame). For UI flows, read the screen between steps with `ui_snapshot` (one call: every visible control + state + `occluded_by`/`clipped` honesty flags) instead of screenshots; `click_control` auto-waits for clickability and refuses disabled/occluded clicks with the reason (1.10+).
...
## Semantic steps suites (1.10+) - the layout-shift-proof alternative
Instead of `events`, save `steps`: a `ui_do` flow of `{click|type|wait|assert|input}` objects addressed by SELECTOR (text/name/class), so the suite keeps passing when the UI moves - a recorded-coordinate replay would silently miss. `playtest op=save name=<n> scene=<res://...> steps=[{"click":"Start"},{"assert":{"node":"StatusLabel","property":"text","equals":"Status: start"}}] asserts=[...]`. `op=run` executes the steps through the game-side ui_do window (each step auto-waits; asserts poll until true), freezes, then evaluates the `asserts` array as usual. Trade-offs, stated plainly: steps suites are NOT frame-exact and capture NO perf stats (record `events` for the perf loop), and the headless CI runner SKIPS them - they are the in-editor regression path. One suite is events OR steps, never both.
...
- `{"type":"screenshot","baseline":"res://tests/baselines/win.png","min_peak_snr":30}` - compares the live frame to a baseline PNG with `Image.compute_image_metrics` **peak_snr** (dB, higher = more alike). First run SAVES the baseline and skips; later runs compare. Reference points measured on 4.6.2/4.7: identical = 1e10, one changed pixel in 64x64 = 41.5, half the frame changed = 9.8, fully different = 6.8 - so the 30 dB default sits above AA/dither noise and below any real regression. Resolution is normalized to the baseline first (a window resize is not a regression). On failure it writes `<baseline>.actual.png` and returns both paths + the numbers, so you can look at them. Needs an RHI: SKIPPED (never failed) under the headless runner. `tolerance` from before 1.12 is no longer used and the result says so.
...
- **`screenshot` asserts need a window.** They pass/fail in an in-editor run (the played game has an RHI) but SKIP under `--headless` (the runner asks the DisplayServer, so a windowed runner DOES compare) - keep node-state / expr asserts for the headless CI path, screenshots for local visual checks.
- **An in-editor run is normally EMBEDDED.** Since 4.4 the editor plays the game inside its Game workspace by default (and "Make Game Workspace Floating" is still embedded). That still has an RHI, so screenshot asserts really do compare - but the frame is sized to the workspace panel, not to your project resolution. A baseline recorded at another size is compared after a resize to the baseline, and the result says so (`[current frame resized to the baseline's WxH]`): record and rerun a baseline from the same placement and panel size, or the resample noise eats the dB margin. `doctor` reports `game_view.mode` and `game_view.placement`.
...
- **One gesture = ONE `simulate_input` call.** Split press / move / release across separate calls and the host's real mouse can interleave between them, corrupting drags and aim. Atomic event lists also record/replay clean.
