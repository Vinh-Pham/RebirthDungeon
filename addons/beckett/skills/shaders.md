...
- **A spatial shader culls back faces by default, and Godot's front face is CLOCKWISE** (opposite of the OpenGL convention). A reversed index buffer, or a stray `render_mode cull_front;`, makes the surface vanish with no error, no warning and a healthy-looking node. `render_probe path=<node>` reports `winding.facing_camera` and the shader's effective `cull_mode`; `render_mode cull_disabled;` confirms it in one edit. See `node3d` for the winding rule in full.
- **A newly written `.gdshader` may not be imported yet.** Rather than creating a throwaway debug shader mid-session (and losing a round trip to the import), edit the shader you already have and hot-swap it: `reload_shader path=res://shaders/terrain.gdshader` reassigns it into the running game without a restart. Compile errors appear in `game_logs` on the next drawn frame, never at load — always read the log after a reload.
