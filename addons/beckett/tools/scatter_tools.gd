...
func _register(registry) -> void:
	registry.register({
		"name": "scatter_nodes",
		"description": "Scatter N copies of a scene or node across a region with random position/rotation/scale — the scriptable Scene Paint: fill a level with props, vegetation or enemies in ONE undoable call (max 1000). source = a res:// .tscn to instance, or a node path/name in the open scene to duplicate. Region shape (2D vs 3D), the rotation/scale ranges and the seed: help(tool=\"scatter_nodes\").",
		"help": "source   a res:// .tscn (instanced per copy) OR a node path / name in the open scene (duplicated per copy)\ncount    how many copies, placed under `parent` (default: the scene root)\nregion   bounds the positions —\n           2D: [x, y, w, h]              for Node2D / Control\n           3D: [x, y, z, w, h, d]        for Node3D; the min corner plus the size\n         Omit region and every copy stacks at the parent origin, which is occasionally what you want and usually not.\nrotation    max random rotation in DEGREES — around Z in 2D, around Y in 3D\nscale_min / scale_max   uniform random scale range (both default 1.0)\nseed        deterministic RNG, so the same call twice gives the same layout\n\nThe whole scatter is ONE undoable action: a bad fill is one Ctrl+Z, not `count` of them. Capped at 1000 copies per call.",
		"destructive": true,
