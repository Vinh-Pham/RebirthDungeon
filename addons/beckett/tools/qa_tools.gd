...
func _register(registry) -> void:
	...
	registry.register({
		"name": "ui_audit",
		"description": "Deterministic layout QA over the RUNNING game's UI — the checks vision does badly and rect math does exactly: offscreen, zero_size, overlap, text_overflow, small_target (touch), mouse_only, plus the keyboard/gamepad focus GRAPH. Returns {issues, counts, checked, focus}. Run it after building UI, before shipping, or when a player says \"I can't click that\". Scope with path=. Every issue kind defined: help(tool=\"ui_audit\").",
		"output_schema": {"type": "object", "properties": {
			"issues": {"type": "array"}, "counts": {"type": "object"},
			"checked": {"type": "object"}, "focus": {"type": "object"},
			"viewport": {"type": "array"}, "truncated": {"type": "boolean"},
		}, "required": ["issues", "counts", "checked", "focus"]},
		"help": "Issue kinds:\n  offscreen         a VISIBLE interactive control lying entirely outside the viewport\n  zero_size         a control with no area to hit\n  overlap           two interactive controls overlapping >= overlap_min_ratio of the smaller one, in the same window, neither an ancestor of the other\n  text_overflow     a Label or Button whose text needs more pixels than the control has. Measured via minimum size; autowrap labels are excluded, because wrapping is not overflow\n  small_target      an interactive rect below touch_min px. Pass 44 for a mobile audit (the platform guideline); 0 = off, and off is the default\n  mouse_only        a BaseButton with focus_mode NONE — keyboard and gamepad can never reach it\n\nThe focus GRAPH (1.11) walks Tab / arrow navigation the way a player without a mouse does:\n  no_initial_focus    focusables exist but nothing HAS focus, so a gamepad user cannot start navigating\n  focus_unreachable   no chain of focus moves from the entry point ever reaches it\n  focus_dead_end      focus can enter but never leave\n\nReturns {issues, counts, checked, focus: {focusable, entry, reachable}}.",
		"readonly": true,
		"input_schema": {"type": "object", "properties": {
			"path": {"type": "string", "description": "subtree root to scope the audit"},
			"touch_min": {"type": "number", "description": "flag interactive targets smaller than this many px (0 = off; 44 = mobile guideline)"},
			"overlap_min_ratio": {"type": "number", "description": "overlap fraction of the smaller control to flag (default 0.25)"},
			"max": {"type": "integer", "description": "issue cap (default 100)"},
		}},
		"handler": Callable(self, "_ui_audit"),
	})
	registry.register({
		"name": "assert_scene",
		...
		"readonly": true,
		"output_schema": {"type": "object", "properties": {
			"pass": {"type": "boolean"}, "reasons": {"type": "array"},
			"scene": {"type": "string"}, "main_scene_set": {"type": "boolean"},
			"node_count": {"type": "integer"}, "types_present": {"type": "array"},
			"missing_types": {"type": "array"},
...
func _ui_audit(args: Dictionary) -> Dictionary:
	...
	var cmd := {"cmd": "ui_audit"}
	for k in ["path", "touch_min", "overlap_min_ratio", "max"]:
		if args.has(k):
			cmd[k] = args[k]
	var r: Dictionary = server.bridge.send_command(cmd, 8000)
	if not bool(r.get("ok", false)):
		return {"error": str(r.get("error", "ui_audit failed"))}
	var out := r.duplicate()
	out.erase("ok")
	out.erase("_id")
	return {"json": out}
...
func _compare_screenshots(args: Dictionary) -> Dictionary:
	...
	var writing_baseline := bool(args.get("save_baseline", false)) or not FileAccess.file_exists(baseline)
	var cmd := {"cmd": "screenshot"}
	if not writing_baseline:
		cmd["scale"] = 0.25
	...
	if base.get_width() > cur.get_width() or base.get_height() > cur.get_height():
		base.resize(cur.get_width(), cur.get_height(), Image.INTERPOLATE_BILINEAR)
	cur.resize(64, 64)
	base.resize(64, 64)
