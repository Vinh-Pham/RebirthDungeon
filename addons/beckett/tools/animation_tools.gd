...
func _register(registry) -> void:
	registry.register({
		...
		"description": "Author and inspect AnimationPlayer clips. Pass 'op' plus its fields. ops: player_create, list, get, create, add_property_track, add_method_track, set_autoplay, remove, play, stop, and the preset_fade / preset_slide / preset_shake / preset_pulse motion generators. Per-op fields, the track-path form and what the presets mint: help(tool=\"animation_manage\").",
		"help": "ops and their fields:\n  player_create {parent?, name?}                     add an AnimationPlayer\n  list {player}                                      libraries + animation names\n  get {player, name}                                 length, loop, tracks, keys\n  create {player, name, length?, loop?}              a new empty clip\n  add_property_track {player, name, (path | node + property), keys:[{time, value}], interp?}\n                                                     keyframe a property. `path` is Godot's \"Node:property\" track form; give node + property instead if that is easier to build.\n  add_method_track {player, name, node, calls:[{time, method, args?}]}\n                                                     fire method calls from the timeline\n  set_autoplay {player, name|\"\"}                     play this clip when the scene runs (\"\" clears it)\n  remove {player, name}                              delete a clip\n  play {player, name, time?} / stop {player}         editor playback state\n  preset_fade | preset_slide | preset_shake | preset_pulse {target, player?, name?, duration?, ...}\n                                                     mint a motion clip. If `player` is omitted it auto-creates a sibling AnimationPlayer, so a one-line call is enough to animate a node that has nothing yet.\n\ninterp: nearest | linear (default) | cubic.",
		"input_schema": {"type": "object", "properties": {
			"op": {"type": "string", "description": "operation (see help)"},
