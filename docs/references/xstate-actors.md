...
- An **invoked actor** is started when its parent machine enters the [state](https://stately.ai/docs/states) it is invoked in, and stopped when that state is exited.
- A **spawned actor** is started in a [transition](https://stately.ai/docs/transitions) and stopped either with a [`stop(...)` action](https://stately.ai/docs/actions#stop-action) or when its parent machine is stopped.

You can visualize your state machines and easily invoke actors in our drag-and-drop Stately editor.

[Read more about actors in Stately’s editor](https://stately.ai/docs/editor-actions-and-actors).

Watch our [“XState: exploring actors” deep dive video on YouTube](https://www.youtube.com/watch?v=Rj7lOvDwcYs).

XState: Exploring actors - YouTube

Tap to unmute

[XState: Exploring actors](https://www.youtube.com/watch?v=Rj7lOvDwcYs) [Stately](https://www.youtube.com/channel/UCqGhQ2VYGE_OZsj8shjV0WA)

![thumbnail-image](https://yt3.ggpht.com/R8-m9nzi3fuLhCHkonORuiSHuay5jWem_7jpk20f6pdObOOcbTP2KWOeoxZZAlhDc64ejax0OjU=s68-c-k-c0x00ffffff-no-rj)

Stately3.54K subscribers

[Watch on](https://www.youtube.com/watch?v=Rj7lOvDwcYs)

## [Actor model](https://stately.ai/docs/actors\#actor-model)

In the actor model, actors are objects that can communicate with each other. They are independent “live” entities that communicate via asynchronous message passing. In XState, these messages are referred to as _[events](https://stately.ai/docs/transitions)_.
...
## [Actor logic](https://stately.ai/docs/actors\#actor-logic)

Actor logic is the actor’s logical “model” (brain, blueprint, DNA, etc.) It describes how the actor should change behavior when receiving an event. You can create actor logic using **[actor logic creators](https://stately.ai/docs/actors#actor-logic-creators)**.
...
## [Creating actors](https://stately.ai/docs/actors\#creating-actors)
...
- `actorLogic`: the [actor logic](https://stately.ai/docs/actors.mdx#actor-logic) to create an actor from
- `options` (optional): actor options

When you create an actor from actor logic via `createActor(actorLogic)`, you implicitly create an [actor system](https://stately.ai/docs/system) where the created actor is the root actor. Any actors spawned from this root actor and its descendants are part of that actor system. The actor must be started by calling `actor.start()`, which will also start the actor system:
...
### [Invoking and spawning actors](https://stately.ai/docs/actors\#invoking-and-spawning-actors)
...
## [Actor snapshots](https://stately.ai/docs/actors\#actor-snapshots)
...
## [Subscriptions](https://stately.ai/docs/actors\#subscriptions)
...
See [persistence](https://stately.ai/docs/persistence) for more details.
...
## [Error handling](https://stately.ai/docs/actors\#error-handling)
...
## [Actor logic creators](https://stately.ai/docs/actors\#actor-logic-creators)
...
### [Actor logic capabilities](https://stately.ai/docs/actors\#actor-logic-capabilities)
...
| [Transition actors](https://stately.ai/docs/transition-actors) | ✅ | ✅ | ❌ | ✅ | ❌ |
| [Observable actors](https://stately.ai/docs/observable-actors) | ❌ | ✅ | ❌ | ✅ | ❌ |
| [Callback actors](https://stately.ai/docs/callback-actors) | ✅ | ✅ | ❌ | ✅ | ❌ |

### [State machine logic (`createMachine(...)`)](https://stately.ai/docs/actors\#createmachine)

You can describe actor logic as a [state machine](https://stately.ai/docs/machines). Actors created from state machine actor logic can:
...
Learn more about [state machine actors.](https://stately.ai/docs/state-machine-actors)
...
Learn more about [promise actors.](https://stately.ai/docs/promise-actors)

### [Transition function logic (`fromTransition(...)`)](https://stately.ai/docs/actors\#fromtransition)

Transition actor logic is described by a [transition function](https://stately.ai/docs/migration#use-actor-logic-creators-for-invokesrc-instead-of-functions), similar to a [reducer](https://stately.ai/docs/cheatsheet#creating-transition-logic). Transition functions take the current `state` and received `event` object as arguments, and return the next state. Actors created from transition logic (“transition actors”) can:
...
Learn more about [transition actors.](https://stately.ai/docs/transition-actors)

### [Observable logic (`fromObservable(...)`)](https://stately.ai/docs/actors\#fromobservable)

Observable actor logic is described by an [observable stream of values](https://stately.ai/docs/actors#fromObservable). Actors created from observable logic (“observable actors”) can:
...
Learn more about [observable actors.](https://stately.ai/docs/observable-actors)

### [Event observable logic (`fromEventObservable(...)`)](https://stately.ai/docs/actors\#fromeventobservable)

Event observable actor logic is described by an observable stream of [event objects](https://stately.ai/docs/transitions.mdx#event-objects). Actors created from event observable logic (“event observable actors”) can:
...
You may choose to use `sendBack` to report caught errors to the parent actor. This is especially helpful for handling promise rejections within a callback function, which will not be caught by [`onError`](https://stately.ai/docs/invoke.mdx#onerror).
...
Learn more about [callback actors.](https://stately.ai/docs/callback-actors)

## [Actors as promises](https://stately.ai/docs/actors\#actors-as-promises)
...
## [Higher-level actor logic](https://stately.ai/docs/actors\#higher-level-actor-logic)
...
## [Custom actor logic](https://stately.ai/docs/actors\#custom-actor-logic)
...
## [Empty actors](https://stately.ai/docs/actors\#empty-actors)
...
## [Actors and TypeScript](https://stately.ai/docs/actors\#actors-and-typescript)

**XState v5 requires TypeScript version 5.0 or greater.**

For best results, use the latest TypeScript version. [Read more about XState and TypeScript](https://stately.ai/docs/typescript)
...
## [Testing](https://stately.ai/docs/actors\#testing)
...
## [Actors cheatsheet](https://stately.ai/docs/actors\#actors-cheatsheet)

### [Cheatsheet: create an actor](https://stately.ai/docs/actors\#cheatsheet-create-an-actor)
...
### [Cheatsheet: state machine logic](https://stately.ai/docs/actors\#cheatsheet-state-machine-logic)
...
### [Cheatsheet: promise logic](https://stately.ai/docs/actors\#cheatsheet-promise-logic)
...
### [Cheatsheet: transition function logic](https://stately.ai/docs/actors\#cheatsheet-transition-function-logic)
...
### [Cheatsheet: observable logic](https://stately.ai/docs/actors\#cheatsheet-observable-logic)
...
### [Cheatsheet: event observable logic](https://stately.ai/docs/actors\#cheatsheet-event-observable-logic)
...
### [Cheatsheet: callback logic](https://stately.ai/docs/actors\#cheatsheet-callback-logic)
...
[Event emitter\\
\\
Previous Page](https://stately.ai/docs/event-emitter) [State machine actors\\
\\
Next Page](https://stately.ai/docs/state-machine-actors)

### On this page

[Actor model](https://stately.ai/docs/actors#actor-model) [Actor logic](https://stately.ai/docs/actors#actor-logic) [Creating actors](https://stately.ai/docs/actors#creating-actors) [Invoking and spawning actors](https://stately.ai/docs/actors#invoking-and-spawning-actors) [Actor snapshots](https://stately.ai/docs/actors#actor-snapshots) [Subscriptions](https://stately.ai/docs/actors#subscriptions) [`waitFor`](https://stately.ai/docs/actors#waitfor) [Error handling](https://stately.ai/docs/actors#error-handling) [Actor logic creators](https://stately.ai/docs/actors#actor-logic-creators) [Actor logic capabilities](https://stately.ai/docs/actors#actor-logic-capabilities) [State machine logic (`createMachine(...)`)](https://stately.ai/docs/actors#createmachine) [Promise logic (`fromPromise(...)`)](https://stately.ai/docs/actors#frompromise) [Transition function logic (`fromTransition(...)`)](https://stately.ai/docs/actors#fromtransition) [Observable logic (`fromObservable(...)`)](https://stately.ai/docs/actors#fromobservable) [Event observable logic (`fromEventObservable(...)`)](https://stately.ai/docs/actors#fromeventobservable) [Callback logic (`fromCallback(...)`)](https://stately.ai/docs/actors#fromcallback) [Actors as promises](https://stately.ai/docs/actors#actors-as-promises) [Higher-level actor logic](https://stately.ai/docs/actors#higher-level-actor-logic) [Custom actor logic](https://stately.ai/docs/actors#custom-actor-logic) [Empty actors](https://stately.ai/docs/actors#empty-actors) [Actors and TypeScript](https://stately.ai/docs/actors#actors-and-typescript) [Testing](https://stately.ai/docs/actors#testing) [Actors cheatsheet](https://stately.ai/docs/actors#actors-cheatsheet) [Cheatsheet: create an actor](https://stately.ai/docs/actors#cheatsheet-create-an-actor) [Cheatsheet: state machine logic](https://stately.ai/docs/actors#cheatsheet-state-machine-logic) [Cheatsheet: promise logic](https://stately.ai/docs/actors#cheatsheet-promise-logic) [Cheatsheet: transition function logic](https://stately.ai/docs/actors#cheatsheet-transition-function-logic) [Cheatsheet: observable logic](https://stately.ai/docs/actors#cheatsheet-observable-logic) [Cheatsheet: event observable logic](https://stately.ai/docs/actors#cheatsheet-event-observable-logic) [Cheatsheet: callback logic](https://stately.ai/docs/actors#cheatsheet-callback-logic)