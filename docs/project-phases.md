...
- **Status:** Phase 4 completed and verified on 2026-09-08: command-only five-dice encounters, frozen inputs/reservations, shared exact damage/stat rules, resources, statuses, shields, cooldowns, deterministic enemy response and scoped outcomes. All 82 JVM tests, simulation-boundary/format checks, desktop compilation and Android debug packaging pass.
- **Next objective:** Connect the battle HUD to the application controller and persist the complete activation before enabling combat in production sessions. Use [the starter combat contract](phase4-combat.md); the movement-only checkpoint exporter deliberately refuses combat sessions.
...
- **Known blockers/unmet gates:** Phase 5 HUD and whole-activation save/restore are unimplemented; production sessions retain movement-only behavior. Desktop combat runtime, Android emulator/device interaction, full iOS AOT/runtime and cross-platform combat replay acceptance remain open. Compilation and Android packaging do not close native runtime gates.
...
- [x] Create one artemis-odb `World` plus `RunSession` per run, with stable IDs, position/player/AI/blocker/vision/health components and authoritative run/floor/counter state.
- [x] Implement a y-up `DungeonGrid` with flattened `IntArray` tile IDs and an O(1) occupancy index; keep static floor/wall tiles out of the ECS.
- [x] Build the `DungeonProcessor` adapter, translate `char[x][y]` into project tiles/spawns, validate reachability and constraints, and bound generation attempts with deterministic failure behavior.
- [x] Establish the game-plan system pipeline in registration order: validation (100), enemy intent (150), movement (200), interaction (300), cleanup (800), visibility (900) and turn finalization (1000); Phase 4 later fills the dice/ability/damage/status slots 400–700 (slot labels per game-plan section 6). Export snapshots/events only after `World.process()` returns.
- [x] Enforce cardinal steps, bounds, terrain and occupancy together. Walking/waiting spend one action; an unlocked door opens without movement for one action; invalid movement or a locked door without a key spends none.
- [x] Reserve hostile-contact handling for Phase 4 without allowing overlap; complete a movement-only slice before enabling dice encounters.
- [x] Implement `DijkstraMap` with `Measurement.MANHATTAN`, explicit blocked terrain and per-query dynamic blockers; use simple deterministic enemy pursuit/idle behavior.
- [x] Implement `FOV.reuseFOV` with `Radius.DIAMOND`, opacity changes, `visibleNow` and persistent explored state; hide live information about unseen enemies from presentation.
- [x] Implement `TurnScheduler` ordered by `(dueTick, insertionSequence, stableActorId)`, starting with 100 ticks per completed activation; persist queue, active actor and tie-break state.
- [x] Implement the serialized `RunController`, accepted command/event counters, automatic-actor guard and explicit `world.process()` steps. Yield only between completed actions if necessary.
- [x] Connect committed snapshots/events to world/HUD rendering, keyboard/D-pad/swipe controls, interpolation and input gating; no rule advances merely because a frame is drawn.
- [x] Add a minimal versioned JSON checkpoint for the actual map, entity DTOs, explored cells, counters, scheduler and full gameplay RNG states; create the serialized alternating-slot save foundation that Phase 6 hardens.
- [x] Rebuild derived indexes/caches on reload and compare a restored continuation against an uninterrupted replay using canonical state/event ordering.
- [x] Test movement/door costs, occupancy/death cleanup, coordinate translation, bounded generation, path blockers, FOV invalidation, initiative ties and command determinism.

Exit criteria:

- [x] A player can explore a generated floor and reach its exit using the shared input commands.
- [x] The same starting state and commands reproduce map, turns, visible state and events, including after a basic save/reload.
- [x] Simulation state is frame-rate independent, snapshots cannot mutate it, and malformed/rejected commands leave it consistent.

**Implementation evidence:** [Phase 3 movement/checkpoint contracts](phase3-movement.md) records the grid/pipeline, provisional AI/vision policy, observed-versus-restore boundary, controls and save format. `GridTurnsTest` and `RunReplayTest` cover the phase invariants, a reachable generated exit with AI, interrupted continuation, persistence failures and the recorded `core/src/test/resources/saves/movement-v1.json` fixture. Only player movement interpolates; visible enemies snap to committed observations. Keys, combat/outcomes, profile/reward saves and platform power-loss hardening remain in their consuming phases. Final verification and platform limits are in the Completion Log.
...
- [x] Author the starter skill IDs, two playable rank profiles, effects/costs/cooldowns and recovery/exhaustion rule before enabling combat. Test SP/MP depletion and legal continuation; town Inn rest does not by itself resolve in-dungeon exhaustion, and no free basic attack is implied.
- [x] Define encounter participation and completion separately from floor objectives and run completion. An empty room/floor or the last hostile in one encounter must not automatically grant a whole expedition result; defeat takes precedence when terminal conditions coincide.
- [x] Add `DiceHand` (five stable dice), `ResourcePools`, `Stats`, `AbilityLoadout`, `StatusSet`, `Shield` and `Cooldowns` alongside existing health/identity components; do not introduce a separate authoritative combat world.
- [x] Implement the chosen contact contract: bumping a hostile opens a dice activation for the current player turn without movement or immediate damage.
- [x] Implement pre-roll skill/target selection: validate the learned active skill, its rank snapshot, equipment legality, range/target, cooldown and combined SP/MP/HP affordability; selection, panel and target changes before the first roll are free and cannot reset anything after it.
- [x] Implement `ROLL_DICE`: lock skill, rank, target, effective attack/mitigation inputs, cost vector and face-weight profile, reserve every required pool cost, and sample five dice independently in stable die order from the locked profile.
- [x] Implement kept-die flags (no RNG, cost or initiative) and batch `REROLL_DICE`: atomically replace a chosen nonempty subset of unkept dice for one of two reroll actions under the same locked profile, keeping every replacement result; reject empty, pre-roll and exhausted-budget rerolls with no budget carry-over and no automatic attack at zero rerolls. `ASSIGN_DIE`, `UNASSIGN_DIE` and the old singular `REROLL_DIE` are removed from this mode's contract.
- [x] Implement `USE_ABILITY` (deduct reserved costs once before effects, consume the whole hand, resolve the locked skill and end the activation exactly once) and `END_TURN` (free pass before rolling; after rolling pay reserved costs and discard the hand; no skill-use training). Keep `USE_ITEM` gated until minimal inventory consumption exists (Phase 5).
- [x] Implement pure Kotlin scoring: pip total 5–30 and exactly one of the eight combination classes with the provisional multiplier table; no overlapping bonuses, four-die straights or wildcards.
- [x] Implement the starter damage rule `B + A + K × P` with flat defense subtracted before the multiplier floor, clamped Protection resistance, shield absorbed last and HP clamped at zero; use exact rational/fixed-point arithmetic at the defined rounding boundaries and one shared resolver for previews and commitment.
- [x] Enforce resource contracts: current/max/reserved HP/MP/SP, effective cost `max(1, ceil((rankCost + flat) × max(0, 1 + pct)))` per positive pool, all pools validated together before rolling, HP payment bypassing mitigation/shield while leaving at least 1 HP, and a skill never financing its own upfront cost.
- [x] Resolve STR/INT/DEX/WIL/LUK and derived stats in dependency order (progression baseline → equipment/status modifiers → derived maxima/attack/defenses → direct derived modifiers) with additive flat/percent aggregation, explicit bounds/rounding and no refill when a maximum rises.
- [x] Implement statuses with source IDs, stacking groups/priorities (refresh/replace rules), durations in affected-actor completed activations, periodic effects before expiry, stat recomputation/clamping and authored regeneration for living actors at eligible activation ends; nothing ticks at application and dice commands never tick or expire effects.
- [x] Fill the dice 400, ability 500, damage 600 and status 700 pipeline slots; resolve status-generated damage before cleanup through the shared synchronous helpers.
- [x] Implement deterministic enemy decisions through the same effect/damage helpers; remove dead actors from occupancy/initiative before selecting the next actor; finalize the player activation once even when the last hostile dies, evaluating defeat before encounter victory; only the application run policy may turn an eligible outcome into run completion.
- [x] Emit immutable ordered combat events and expand canonical replay fixtures: exhaustively classify all 7,776 ordered five-die hands across the eight combination classes; pin one initial roll plus two subset rerolls, frozen inputs, held faces, weighted-probability boundaries, damage distributions, HP bounds, invalid commands and terminal states.
- [x] Author the starter slice content: sword attack skill at two illustrative ranks (fair and weighted profiles), one enemy response, one skill buff and one enemy stat debuff; add the authored defensive skill, a mana skill and an illustrative HP-cost case as their effects and durations are authored. Stage Smash and Combat/Sword Mastery as their values become available. **Disposition:** these unvalued progression extensions remain deferred; the authored starter skill set and rationale are in [phase4-combat.md](phase4-combat.md).

Exit criteria:

- [x] One hero and one enemy can resolve an encounter to victory or defeat through plain JVM command tests using the starter sword skill with fair and weighted profiles.
- [x] Identical state, streams and commands produce identical rolls, kept dice, damage, turn order and events.
- [x] Invalid commands, extra rolls or rerolls, post-lock skill/target/panel changes and turn-finalization edge cases cannot create resources, rerolls, damage or extra turns.

Verification (2026-09-08, macOS arm64, wrapper 9.5.1 / daemon JVM 25): `./gradlew --console=plain --info :core:check :lwjgl3:compileKotlin :android:checkDebugDuplicateClasses :android:assembleDebug` passed, **82/82 JVM tests** with no failures/errors/skips. Evidence: `core/build/test-results/test/`, `core/build/reports/tests/test/index.html`, `android/build/outputs/apk/debug/android-debug.apk`, `.firecrawl/phase4-verification.log`. `game/combat/{CombatRulesTest,CombatSimulationTest}.kt` under the test root verifies exhaustive classifications, fair/weighted full damage histograms, a seed-71 roll/reroll/canonical fixture, command rejection conservation, payment/preview parity, depletion/recovery, self-buff/enemy-debuff timing, stacking, shield/cooldown expiry, maximum clamping, fair/weighted victory, defeat and simultaneous-terminal precedence. Content validation tests cover the new fields; the content-v1 movement replay runs against its original pinned catalog. Production combat/HUD, mid-activation checkpointing and native runtime acceptance remain Phase 5, not Phase 4 claims.
...
| Starter content identities and scope | Phase 2 schema; Phase 4 values; Phase 7 acquisition | Starter authored; acquisition open | Content v2 provides sword F/E, fortify, focus, spark, blood and enemy_strike; [starter contract](phase4-combat.md) records caps/effects. Smash/masteries and acquisition/training remain later authored content. |
| Dungeon resource exhaustion and recovery | Phase 4 | Starter authored 2026-09-08 | Living owner completion regenerates 0 HP / 1 MP / 2 SP; pre-roll passing remains legal, with enemy responses. See [starter contract](phase4-combat.md). Future town fees/access remain separate. |
| Encounter, floor and run completion | Phase 4; extend Phase 10 | Starter authored 2026-09-08 | Contacted/targeted/responding hostiles participate; empty exploration cannot win. Resolve owner statuses and cleanup, then defeat before encounter victory. Encounter victory grants no expedition result; application outcome policy remains later work. |
...
| 3 — Grid simulation, turns, and basic checkpoints | 2026-09-08 | Gradle wrapper 9.5.1, daemon JVM 25, macOS arm64; ordinary JVM tests; desktop LWJGL3; Android debug packaging | `./gradlew :core:check :lwjgl3:compileKotlin :android:checkDebugDuplicateClasses :android:assembleDebug --console=plain` PASS: 58/58 tests, simulation boundary/format checks, desktop compilation and Android duplicate-class/debug APK gates. Test reports: `core/build/test-results/test/`, `core/build/reports/tests/test/index.html`; APK: `android/build/outputs/apk/debug/android-debug.apk`. Isolated `REBIRTH_CHECKPOINT_DIR` + `REBIRTH_AUTODEMO=1 ./gradlew :lwjgl3:run --console=plain` PASS twice, including a fresh process resuming Actions 4 / Turns 8 / Tick 400 and continuing to 8 / 16 / 800; clean exits. Visually inspected `assets/screenshots/rebirth-phase3-entry-1788875902612.png` and `rebirth-phase3-after-reload-1788875908714.png`; the final before/after reload PNGs are byte-identical. Recorded schema-1 desktop fixture: `core/src/test/resources/saves/movement-v1.json`. Logs/evidence: `.firecrawl/phase3-logs/`. No new dependencies; no Android interaction, iOS AOT/runtime or cross-platform replay claim. |
...
- **2026-09-08 — Title screen wired from the Penpot design (user-directed; Phase 7 menu slice started early, no phase boxes closed).** Added `presentation/screens/TitleScreen.kt` reproducing the Penpot board "Title Screen" (1280×720 design space on a FitViewport): navy gradient `#273052→#0B0E1A`, edge vignette, torch glow, Press Start 2P gold `#EFC75E` title with hard pixel shadow, five-dice 1–5 divider, and the bottom-center Start Game button navigating to `DungeonScreen`. Fonts ship as generated bitmap assets (`assets/ui/title-52/20.fnt+.png`) from the OFL-licensed TTF kept with its license in `tools/fonts/`; regenerate with `java tools/MakeTitleFont.java` (Java2D, no new runtime dependency). Background layers, dice strip and button patches are screen-owned procedural Pixmaps. Navigation rewired: LoadingScreen is a pure loading/failure screen that hands off to TitleScreen; the dungeon Menu action and the auto-demo loop return to TitleScreen (repeatable menu⇄dungeon transitions preserved). Fixed an entry wedge found during desktop verification: an `UnsupportedCheckpoint` from stale-save `resume()` left `controller=null` with no fallback — `RunServices` now treats version-incompatible checkpoints as no restorable state (files preserved) and `DungeonScreen.show()` falls back to a fresh run; the save-path conflict still surfaces through the controller's retryable failure lane. Verification: `:core:check :lwjgl3:compileKotlin` PASS (82/82 tests, boundary/format clean) after all changes, plus live desktop auto-demo (`REBIRTH_AUTODEMO=1 :lwjgl3:run`, exit 0): Loading→Title→Dungeon→Title→Dungeon with captures `assets/screenshots/rebirth-menu-1788929602679.png` and `rebirth-menu-1788929606903.png` matching the board; fresh checkpoints written to `~/.rebirthdungeon/saves/`. Pre-existing unrestorable v1-era slots from 2026-09-07 22:36 (current build refuses them) were archived by hand to `/tmp/rebirth-stale-saves/` (not deleted by the game). Pixel-font source metrics differ slightly from Penpot's renderer (832px vs 780px title line), absorbed by centering.

- **2026-09-08 — Phase 4 completed.** Added dice/ability/damage/status slots to the existing World, with pure scoring, exact fraction stat derivation, fixed-point costs, shared preview/commit damage, status stacking and typed detached observations. Starter recovery is 0 HP / 1 MP / 2 SP per living owner completion; no regeneration on dice commands and no free attack. Hostile contact/selection starts explicit encounter participation; defeat beats simultaneous encounter victory, and neither encounter victory nor a floor exit grants expedition results. Dead actors leave occupancy and the entire dead set leaves initiative before selection. Added sword F/E, fortify, focus, spark, mixed HP-cost blood and an enemy debuff response to content v2; preserved the actual v1 catalog for the recorded desktop movement fixture. The old movement save exporter rejects combat sessions to prevent lost dice/reservations/statuses; production sessions keep `combatEnabled=false` until Phase 5 supplies the complete codec/HUD/controller path. `USE_ITEM`, Smash and Combat/Sword Mastery retain their documented later-phase gates. Verification: 82 tests plus boundary/format, desktop compile and Android packaging; no desktop combat launch, Android interaction, iOS build/runtime or cross-platform combat replay claim. Next action: implement Phase 5 without removing the save gate until all combat state is durable.

- **2026-09-08 — Phase 3 implemented and verified.** Evolved the existing World/RunSession into the ordered 100/150/200/300/800/900/1000 pipeline with stable-ID ECS actors, O(1) occupancy, opening/wait costs, hostile-contact rejection, Manhattan pursuit, diamond FOV and remembered terrain. Bounded floor generation now validates the exit route and off-route enemy placement. Added deterministic initiative and an owner-thread/session-checked `RunController`; player/automatic action boundaries checkpoint serially, save failures gate input, and invariant/automatic-loop failures halt. Added explicit LibGDX JSON DTOs, checksum/revision alternating slots, strict restore validation, lossless RNG/long values, canonical replay comparison and a recorded desktop save fixture. Application-owned controllers/writer survive menus; the screen consumes only observations and supports tap/swipe/keyboard/D-pad/wait, reload, new runs and retry-save. Fresh-process desktop continuation and identical reload screenshots are verified in the Completion Log, alongside 58 JVM tests and Android debug packaging. [phase3-movement.md](phase3-movement.md) documents the first movement save schema and limits. Next: Phase 4; keep exhaustion and encounter-versus-run outcome rules open until authored. Native suspension/power-loss recovery, migrations and current mobile runtime/replay evidence remain later gates.
