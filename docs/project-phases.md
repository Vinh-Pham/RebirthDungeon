...
- **Status:** Phase 1 completed and verified on 2026-09-04 (see Completion Log); Phase 2 is not started. The 2026-09-06 user-directed Kotlin migration, Kotlin-DSL build scripts, and full liftoff dependency adoption changed sources and the build after that runtime evidence; the latest KTX work note also records a desktop auto-demo launch. Android interaction and iOS AOT/runtime evidence still predate those changes.
- **Next objective:** Stable content/entity IDs and command/event values, the project `RandomSource` interface with the Juniper `AceRandom` adapter, and strict catalog loading under `data/content` into immutable `game/content` definitions — including five-dice scoring/face-weight profiles and starter skill/stat/cost/status definitions. Give demonstration skills stable IDs and visible prototype caps; defer town catalogs and full progression tables until their consuming phases.
...
- **Known blocker:** None for Phase 2's JVM-only work. The September 6 KTX-deepening note records a desktop auto-demo run after migration/dependency changes; it does not verify current mobile runtime behavior. Android emulator/device interaction and full iOS AOT/simulator validation with the current dependency set remain open. Earlier mobile launches and current compilation/debug packaging are distinct evidence; re-run the mobile batteries before relying on on-device behavior.
...
- [ ] Define stable content/entity IDs, Kotlin command/result/event values, version conventions and immutable snapshot conventions without presentation or backend types.
...
- [ ] Add Jackson-bound JSON DTOs and immutable definitions under `game/content` with strict loaders/DTOs/validation under `data/content`, backed by `ContentRepository`; validation is explicit rather than assumed from JSON parsing (unknown fields/enum values already fail the Jackson binding; keep that strictness).
