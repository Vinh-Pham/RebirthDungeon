...
- **Current phase:** Phase 4 — Deterministic five-dice combat.
- **Status:** Phase 3 completed and verified on 2026-09-08: authoritative movement/doors/occupancy, deterministic AI/turns, observed fog, and alternating-slot checkpoints with reload/replay parity. All 58 JVM tests, simulation-boundary checks, desktop compilation/runtime and Android debug packaging pass; see Completion Log and [movement/checkpoint contracts](phase3-movement.md).
- **Next objective:** Phase 4's starter recovery/exhaustion and encounter-completion contracts, followed by command-only five-dice combat in the existing World. Preserve the pinned catalog/RNG, scheduler, observations and checkpoint boundaries; do not treat reaching a floor exit as an expedition reward grant.
...
- **Known blocker:** Before enabling combat, author the in-dungeon recovery/exhaustion rule and distinguish encounter, floor and run completion as Phase 4 requires. Android emulator/device interaction, full iOS AOT/runtime and cross-platform replay acceptance with the current sources remain open. Phase 3 verifies desktop runtime and Android debug packaging, not those mobile runtime gates.
...
Phases 0–3 are complete as recorded below; their numbers were not changed by the 2026-09-05 realignment. Add a row only after all phase tasks and exits are verified; include the date, target and exact evidence.
