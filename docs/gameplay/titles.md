...
**Defold scope:** milestone 6, after the playable Alby loop. No title system is implemented. The [game plan](../game-plan.md) governs delivery; “first slice” below means the title feature’s own later introduction.
...
Changing a resource maximum does not restore current HP/MP/SP. Lowering a maximum clamps the current pool; re-equipping a bonus does not restore the lost amount. Title effects do not award AP, rank up skills, change authoritative random sampling, grant free actions, or bypass minimum skill costs unless a later supported rule explicitly defines such behavior.
...
At the outcome boundary, resolve retained rewards and progression using character.md and quests.md, then evaluate relevant title conditions from the committed facts and retained evidence. A successful-clear condition requires the boss and committed treasure return to town. Claimed rewards, EXP and committed progression survive defeat/abandonment. Define each additional title’s evidence policy explicitly; do not infer successful-clear credit from retained kills or independently save pending counters.
...
Before implementation, settle the actual catalog and balance, any title-specific run-evidence retention, discovery/spoiler rules, and coupon sources. Acceptance checks should cover:
...
## Defold title integration

Use versioned Lua definitions under proposed `game/content/titles/` and pure predicates/modifier rules under `game/domain/`. Store earned/discovered IDs, evidence, selections and favorites in the hero envelope. Reuse the shared stat resolver; never mutate catalog definitions or derive ownership from a GUI label.

Druid renders the collection and previews through a Monarch panel. Quest/Ink can request a validated award/coupon command; the initiating candidate commits award, consumption, stats and evidence through DefSave. Event announces only after saving. No notification callback independently grants titles. Title selection consumes no RNG; cosmetic effects may use Tweener without changing outcome.

Use Lester for predicates, source removal, no-refill rules, duplicate coupons/claims and failed-write isolation. Engine tests cover save/resume, GUI focus and character switching. See [architecture](../architecture.md), [library integration](../turn-based-rpg-battle-libraries.md), and [verification](../verification.md).
