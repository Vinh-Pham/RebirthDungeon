...
This is a design specification for planned gameplay, based on **Mabinogi**. It complements [skills.md](skills.md), [stats.md](stats.md), [character.md](character.md), [battle.md](battle.md), and the [game plan](../game-plan.md). It does not claim enchanting is implemented. The Rebirth Dungeon rules and sample values below are proposed defaults; the first slice deliberately limits the failure economy.

**Defold scope:** milestone 6, after the playable Alby loop. “First slice” below means enchanting’s own later introduction. Use the [architecture](../architecture.md) for transactions and the game plan for precedence.
...
Enchanting and burning are **between-run actions** in the initial design. Only committed inventory can be used; unclaimed dungeon offers must be claimed through the normal reward transaction first. Neither action is available while a run is active, and neither advances battle initiative.

An installed enchant belongs to the equipment instance. Moving that item between inventory and an equipment slot preserves its enchants. Bonuses apply only while the item is equipped and the relevant conditions are met. Inventory ownership follows [inventory.md](inventory.md); multiplayer trading and binding are deferred; enchanting does not introduce multiplayer entrusting or account-wide sharing.
...
Resource-maximum bonuses do not refill HP, MP, or SP; reductions clamp current pools under stats.md. Enchants do not grant skill ranks, AP, extra turns, or hidden critical chances. They affect battle through the existing effective stats and costs. Skill-rank base damage remains owned by the selected skill.
...
The initial recipe consumes **one scroll, one powder, and the Enchant skill rank's MP cost per accepted attempt**. A provisional cost of 6 MP matches the reference's scale; final costs use stats.md's resource-cost resolver. Use the town’s explicit 10-gold restoration service for the MP recovery loop before enabling this recipe. Opening the menu or returning from an animation never restores mana.
...
Add a dedicated persisted enchanting RNG stream for application checks, variable values, and burn recovery. Keep it separate from dungeon combat, loot, and cosmetic streams. Use the game plan’s Defold RNG adapter: explicitly seeded PCG32, versioned 32-bit seed fields and raw-draw count, restored by counted replay. This additional stream is introduced only with the enchant milestone and a versioned save contract. UI previews consume no randomness; rejected commands leave the stream unchanged.
...
Still open: loot and shop availability, rank/powder chance tables, variable ranges, MP costs and training/AP pacing. Basic weapon durability/repair already belongs to the first-loop plan; enchant-specific durability penalties, Protect Scroll, protection consumables, scroll expiration, elemental enchants, binding/personalization, multiplayer entrusting, and calendar/event bonuses remain outside the first slice.

## Defold enchant integration

Use versioned Lua enchant/recipe definitions in proposed `game/content/` and pure eligibility/cost/effect rules in `game/domain/`. Druid/Monarch panels preview materials, replacement, success chance and destructive burning. The session computes a copied candidate with the dedicated RNG adapter and commits inventory, costs, training, resolved values, claim/operation identity and RNG descriptor in one DefSave envelope.

A failed save discards candidate state and RNG; uncertain writes require read-back before retry. Event notifications and Tweener reveals present the saved result only. Ink can open the service or request a command, never perform a second roll. Quest evidence, when applicable, joins the same transaction through its adapter.

Lester fixtures prove chance boundaries, prefix/suffix independence, maximum output-capacity checks, no-refill source changes and failed-save retry. Engine tests prove native RNG continuation, actual persistence, GUI confirmation/focus and character switching. See [library integration](../turn-based-rpg-battle-libraries.md) and [verification](../verification.md).
