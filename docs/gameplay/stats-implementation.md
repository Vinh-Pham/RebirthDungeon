...
Run entry freezes profile/growth, learned ranks, equipment contributions and title sources in a versioned source snapshot. Growth earned during the run applies after leaving. Existing weapon breakage still applies a separately named live penalty. Statuses are live sources. The first roll freezes costs, attack inputs and target mitigation; reload and rerolls preserve the snapshot. All costs are checked together and paid once before effects (or on a paid pass). HP payments bypass shields and leave at least 1 HP. Active positive costs use the design's ceiling/minimum-one formula; zero-cost pools remain zero. Mana Recovery now requires 5 SP up front.
...
At each completed owner activation: process periodic effects in stable group/source order, decrement eligible durations, remove expired effects, recompute/clamp, then regenerate if alive. Self-applied effects skip that activation end; effects applied outside the owner's activation first tick at its upcoming activation end. No tick occurs for rolls, rerolls, menus or exploration. Defeated actors cannot be revived by ordinary periodic recovery. Temporary statuses clear on run exit/abandonment/defeat. Final Hit, counters and Mana Shield retain their existing specialized combat timing and save compatibility; Final Hit and legacy Defense are resolved as named sources.
