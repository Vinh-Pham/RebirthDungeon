...
## Per-battle combat log — September 19, 2026

- Added the shared action formatter and movable HeroUI/wmkit Combat Log window, both entry points, expandable details, numbered enemies, battle selection, paused auto-follow and concise live announcements. The compact preview uses the same action summaries.
- Schema 6 preserves available old events, marks migrated/truncated history incomplete and backs up version-5 saves. Transactions archive victory, defeat and abandonment once, transfer memory battle records, retain 20 completed battles with a 1,000-event cap, and preserve history through reload/rebirth. See [Combat log](combat-log.md).
- Final lint, typecheck, production build and whitespace checks passed. All **185 unit/component tests passed**; coverage: **94.48% statements, 90.47% branches, 97.65% functions, 96.42% lines**, satisfying the unchanged 90% gates. One earlier coverage run timed out in an existing skill UI test while browsers were running; the isolated final run passed.
- The clean affected browser run passed **13 journeys across Chromium and WebKit**, with three existing WebKit skips. It covers both log entry points, selection, expansion, Escape/focus restoration, movement/resizing, 320×568 bounds, paused following/jump, reload/read-only browsing, existing battle controls and window navigation. An earlier overlapping Firefox run removed trace artifacts; the clean rerun above supersedes it.
- **Firefox was attempted and failed before navigation** with “Could not find profile folder.” It remains configured and is not counted as verified.

The current unit suite covers individual turn order, fractional resources, items and effect timing, 1,000 dungeon seeds, the complete dungeon/reward loop, progression, inventory, quests/RP, save migrations, and persistence rollback/fallback. Coverage gates remain 90% statements, branches, functions, and lines across rules and persistence. Browser tests use real input and a read-only test-mode snapshot; fixtures are seeded through IndexedDB, without production mutation shortcuts.

## Individual-turn battle implementation — September 19, 2026

- Replaced dice with fixed Speed order, optional item plus main action, owner-boundary effects, and resource-aware enemy commands. Added the HeroUI/Tailwind battle panel, shared previews, event feedback, schema-5 migrations, pure-rand continuation, Zod boundaries, and fast-check invariants. The [battle contract](turn-based-plan.md) records the rules and Firecrawl documentation sources.
- `pnpm typecheck --filter=client` and `pnpm lint --filter=client` passed. Scoped oxfmt and `git diff --check` passed. Production builds completed during browser verification.
- `pnpm test:coverage --filter=client`: **168 tests passed**. Coverage: **94.36% statements, 90.54% branches, 97.46% functions, 96.30% lines**. Generated cases check immutable previews, bounded resources, fixed order, invalid/stale commands, exact seeded reloads, RNG continuation, and failed-write retry equivalence. Exact regressions cover all mastery ranks, legacy versions 1–4, borrowed RP actors, counter kills, Mana Shield upkeep, terminal events, and writer-lock loss.
- Chromium game/quest journeys passed across the full run and focused reruns: dungeon/reload/boss/chest return, hunting rewards, Aren’s memory, shops, skill acquisition, catalog scrolling, mixed HP/SP costs, potion timing, and the HUD. The mixed-cost journey was updated from its obsolete reservation assertion and passed on the focused rerun; the other nine journeys passed in the full run.
- The new focused battle journey passed in **Chromium and WebKit**: all four menus, keyboard activation, item allowance shared with Inventory and retained on reload, Defend then Skill/Attack, a read-only second tab, and layouts at **320×800 and 320×568**. Narrow-screen screenshots were visually inspected; the battle panel stays above the HUD, with a visible turn-order strip and scrolling action content.
- **Firefox was attempted but could not launch**: “Could not find profile folder,” before game navigation. Retrying with a separate temporary directory produced the same failure. This matches the existing host limitation; Firefox remains enabled in configuration and is not counted as verified.

Earlier entries below describe historical implementations and their checks; dice, reservation, and older-schema results are superseded by the implementation above.
...
- Lint, TypeScript, production build, touched-code Oxfmt checks, and whitespace checks passed.
...
- Lint, TypeScript, Oxfmt, whitespace checks, and production build: passed.
...
- TypeScript, lint, Oxfmt checks, and production build: passed.
