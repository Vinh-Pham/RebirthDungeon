...
The scan includes application code, tests, build configuration, and JavaScript scripts. Browser globals are available to application code; tests and tooling also receive Node globals. Generated output, dependencies, Firecrawl caches, and `.kilo`/`.codex` worktrees are excluded. Gitignore exclusions also apply. Prettier remains responsible for formatting and `tsc --noEmit` for type checking; experimental Oxlint type checking is not enabled.
