...
// (Kotlin port of the original Java utility; behavior is unchanged.)
 ...
 *    environment variable set to `0` (if it was not already). This is required for
...
object StartupHelper {
    ...
    /**
     * Applies the utilities as described by [StartupHelper]'s documentation.
     *
     * All environment variables are copied to the child JVM process (if it is spawned), as
     * specified by [ProcessBuilder.environment]; the same applies for
     * the system properties.
     *
     * Usage: `if (StartupHelper.startNewJvmIfRequired()) return;`
     *
    ...
    fun startNewJvmIfRequired(inheritIO: Boolean): Boolean {
        ...
        if (osName.contains("windows")) {
            ...
            val programData = System.getenv("ProgramData") ?: "C:\\Temp" // if ProgramData isn't set, try some fallback.
    ...
    fun startNewJvm0(isMac: Boolean, inheritIO: Boolean): Boolean {
        ...
        if (mainClass == null) {
            ...
            if (trace.isNotEmpty()) mainClass = trace[trace.size - 1].className
            else {
    ...
    private fun getProcessID(isMac: Boolean): Long {
        // LWJGL maps the two platform getpid bindings to different numeric types.
        return (if (isMac) LibC.getpid() else UNISTD.getpid()).toLong()
    }
