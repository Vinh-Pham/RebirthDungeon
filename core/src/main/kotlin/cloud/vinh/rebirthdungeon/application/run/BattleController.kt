...
class BattleController(private val simulation: BattleSimulation, private val repository: CheckpointRepository,
    ...
    private fun checkOwner() = check(Thread.currentThread() === owner) { "BattleController must stay on its owning render thread" }
    ...
    fun battleView(presenting: Boolean = false): BattleView? {
        ...
        val blocked = failure?.let { "Save failed: $it — Retry save" } ?: when {
            combat.outcome != null -> "${combat.outcome} — returning to exploration"
    ...
    fun observe(): BattleObservation = simulation.observe(observedEvents)
    ...
    fun restoreExport() = simulation.restoreExport()
