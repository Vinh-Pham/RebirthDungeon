...
class CombatLoadout(learned: Map<ContentId, String>, equipment: Set<String>, modifiers: List<StatModifier> = emptyList(),
    val masteryRank: String = "F", val costFlat: ResourceVector = ResourceVector(0, 0, 0), val costPercent: CostPercent = CostPercent()) {
    init { StaminaRules.index(masteryRank); require(listOf(costFlat.hp, costFlat.mp, costFlat.spTenths, costPercent.hp, costPercent.mp, costPercent.sp).all { it in -1_000_000..1_000_000 }) }
...
data class ResolvedAbility(val selection: Selection, val rank: SkillRank, val definition: SkillDefinition,
