...
class DungeonInput(private val stage: Stage, private val hud: BattleHud, private val world: WorldInputHandler,
    ...
    fun cancel() { gestureIdentity.clear(); ownership.cancel(); stage.cancelTouchFocus(); world.cancelGesture(); hud.inputRevision = null; hud.inputToken = null }
    override fun touchDown(x: Int, y: Int, pointer: Int, button: Int): Boolean {
        val owner = ownership.down(pointer, if (hud.owns(x, y)) InputOwnership.Owner.UI else InputOwnership.Owner.WORLD)
        if (owner != InputOwnership.Owner.BLOCKED) {
            ...
            if (owner == InputOwnership.Owner.UI) stage.touchDown(x, y, pointer, button) else world.touchDown(x, y, pointer, button)
        }
        return true
    }
    override fun touchDragged(x: Int, y: Int, pointer: Int): Boolean {
        when (ownership.owner(pointer)) {
            ...
            InputOwnership.Owner.WORLD -> world.touchDragged(x, y, pointer)
            else -> Unit
        }; return true
    }
    override fun touchUp(x: Int, y: Int, pointer: Int, button: Int): Boolean {
        ...
        when (ownership.up(pointer)) {
            ...
            InputOwnership.Owner.WORLD -> world.touchUp(x, y, pointer, button)
            else -> return true
        }
    ...
    override fun keyDown(keycode: Int): Boolean {
        ...
        return hud.key(keycode, Gdx.input.isKeyPressed(Input.Keys.SHIFT_LEFT) || Gdx.input.isKeyPressed(Input.Keys.SHIFT_RIGHT)) || stage.keyDown(keycode) || world.keyDown(keycode)
    }
