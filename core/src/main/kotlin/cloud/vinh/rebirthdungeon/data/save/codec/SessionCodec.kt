...
class SessionCodec {
    ...
    fun encode(s: SessionRestore): String {
        val e = s.exploration
        return jsonText(listOf(4, listOf(s.version.schema, s.version.content, s.version.rules), s.worldVersion, s.seed, s.expedition, s.operation, s.town,
            ...
            s.battle?.let(battle::encode), s.encounter, CheckpointCodec.random(s.random), s.notice, s.lastBattleEvents.map(BattleEventCodec::encode)))
    }
    fun decode(text: String): SessionRestore {
        val r = JsonReader().parse(text)
        require(r.isArray && r.size > 0)
        if (r[0].int() != 4) throw UnsupportedCheckpoint("Unsupported session version")
        r.record(19)
        ...
        return SessionRestore(ContentVersion(v[0].int(), v[1].int(), v[2].int()), r[2].int(), r[3].long(), r[4].long(), r[5].long(), r[6].bool(),
            ...
            if (r[14].isNull) null else battle.decode(r[14].text()), if (r[15].isNull) null else r[15].text(), CheckpointCodec.readRandom(r[16]), r[17].text(), r[18].rows().map(BattleEventCodec::decode))
    }
}
